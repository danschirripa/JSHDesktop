 ...
 * $State: Exp $
 */
package jshdesktop.javax.media.jai;
 ...
 *
 * <p>
 * A perspective transformation is capable of mapping an arbitrary quadrilateral
 * into another arbitrary quadrilateral, while preserving the straightness of
 * lines. Unlike an affine transformation, the parallelism of lines in the
 * source is not necessarily preserved in the output.
 *
 * <p>
 * Such a coordinate transformation can be represented by a 3x3 matrix which
 * transforms homogenous source coordinates <code>(x,&nbsp;y,&nbsp;1)</code>
 * into destination coordinates <code>(x',&nbsp;y',&nbsp;w)</code>. To convert
 * back into non-homogenous coordinates (X, Y), <code>x'</code> and
 * <code>y'</code> are divided by <code>w</code>.
 *
 * <pre>
...
public final class PerspectiveTransform implements Cloneable, Serializable {
	...
	/**
	 * Scales the matrix elements so m22 is equal to 1.0. m22 must not be equal
	 * to 0.
	 */
	...
	private static final void getSquareToQuad(double x0, double y0, double x1,
	...
	public static PerspectiveTransform getSquareToQuad(float x0, float y0,
		...
		return getSquareToQuad((double) x0, (double) y0, (double) x1,
				(double) y1, (double) x2, (double) y2, (double) x3, (double) y3);
	}
	...
	public static PerspectiveTransform getQuadToSquare(float x0, float y0,
			float x1, float y1, float x2, float y2, float x3, float y3) {
		return getQuadToSquare((double) x0, (double) y0, (double) x1,
				(double) y1, (double) x2, (double) y2, (double) x3, (double) y3);
	}
	...
	public static PerspectiveTransform getQuadToQuad(double x0, double y0,
		...
		PerspectiveTransform tx1 = getQuadToSquare(x0, y0, x1, y1, x2, y2, x3,
				y3);

		PerspectiveTransform tx2 = getSquareToQuad(x0p, y0p, x1p, y1p, x2p,
				y2p, x3p, y3p);
	...
	public static PerspectiveTransform getQuadToQuad(float x0, float y0,
			float x1, float y1, float x2, float y2, float x3, float y3,
			float x0p, float y0p, float x1p, float y1p, float x2p, float y2p,
			float x3p, float y3p) {
		return getQuadToQuad((double) x0, (double) y0, (double) x1,
				(double) y1, (double) x2, (double) y2, (double) x3,
				(double) y3, (double) x0p, (double) y0p, (double) x1p,
				(double) y1p, (double) x2p, (double) y2p, (double) x3p,
				(double) y3p);
	}
	...
	/**
	 * Retrieves the 9 specifiable values in the 3x3 affine transformation
	 * matrix into an array of double precision values. The values are stored
	 * into the array as { m00 m01 m02 m10 m11 m12 m20 m21 m22 }.
	 *
	 * @param flatmatrix
	 *            The double array used to store the returned values. The length
	 *            of the array is assumed to be at least 9.
	 * @throws ArrayIndexOutOfBoundsException
	 *             if flatmatrix is too small
	 * @deprecated as of JAI 1.1 Use double[][] getMatrix(double[][] matrix)
	 *             instead.
	 */
	@Deprecated
	...
	/**
	 * Retrieves the 9 specifiable values in the 3x3 affine transformation
	 * matrix into a 2-dimensional array of double precision values. The values
	 * are stored into the 2-dimensional array using the row index as the first
	 * subscript and the column index as the second.
	 *
	 * @param matrix
	 *            The 2-dimensional double array to store the returned values.
	 ...
	 * equivalent to calling concatenate(T), where T is an PerspectiveTransform
	 ...
	 * equivalent to calling concatenate(R), where R is an PerspectiveTransform
	 * represented by the following matrix:
	 * 
	 * <pre>
	 ...
	 * equivalent to calling concatenate(S), where S is an PerspectiveTransform
	 * represented by the following matrix:
	 * 
	 * <pre>
	 * 	[   sx   0    0   ]
	...
	/**
	 * Sets this transform to a rotation transformation about a specified point
	 * (x, y). This is equivalent to the following sequence of calls:
	 *
	 * <pre>
	 * setToTranslate(x, y);
	 * rotate(theta);
	...
	/**
	 * Sets this transform to a scale transformation with scale factors sx and
	 * sy. The matrix representing this transform becomes:
	 * 
	 * <pre>
	 * 	[   sx   0    0   ]
	 * 	[   0    sy   0   ]
	...
	/**
	 * Sets this transform to a shearing transformation with shear factors sx
	 * and sy. The matrix representing this transform becomes:
	 * 
	 * <pre>
	 * 	[   1  shx    0   ]
	 * 	[ shy    1    0   ]
	 ...
	 *
	 * @param matrix
	 *            The 2D double array to be used for setting this transform. The
	 *            array is assumed to be at least 3x3.
	 * @throws IllegalArgumentException
	 *             if matrix is null
	 * @throws ArrayIndexOutOfBoundsException
	 *             if matrix is too small
	...
	/**
	 * Returns a new PerpectiveTransform that is the adjoint, of the current
	 * transform. The adjoint is defined as the matrix of cofactors, which in
	 * turn are the determinants of the submatrices defined by removing the row
	 * and column of each element from the original matrix in turn.
	 *
	 * <p>
	 * The adjoint is a scalar multiple of the inverse matrix. Because points to
	 * be transformed are converted into homogeneous coordinates, where scalar
	 * factors are irrelevant, the adjoint may be used in place of the true
	 * inverse. Since it is unnecessary to normalize the adjoint, it is both
	 * faster to compute and more numerically stable than the true inverse.
	 */
	...
	/**
	 * Transforms the specified ptSrc and stores the result in ptDst. If ptDst
	 * is null, a new Point2D object will be allocated before storing. In either
	 * case, ptDst containing the transformed point is returned for convenience.
	 * Note that ptSrc and ptDst can the same. In this case, the input point
	 * will be overwritten with the transformed point.
	 *
	 * @param ptSrc
	 ...
	 *            source array.
	 * @param dstOff
	 *            The offset to the location where the first transformed point
	 *            object is stored in the destination array.
	 * @param numPts
	...
	public void transform(Point2D[] ptSrc, int srcOff, Point2D[] ptDst,
		...
		while (numPts-- > 0) {
			...
			if (w == 0) {
				...
				dst.setLocation((m00 * x + m01 * y + m02) / w, (m10 * x + m11
						* y + m12)
						/ w);
			}
		}
	...
	/**
	 * Inverse transforms the specified ptSrc and stores the result in ptDst. If
	 * ptDst is null, a new Point2D object will be allocated before storing. In
	 * either case, ptDst containing the transformed point is returned for
	 * convenience. Note that ptSrc and ptDst can the same. In this case, the
	 * input point will be overwritten with the transformed point.
	 * 
	 * @param ptSrc
	...
	public Point2D inverseTransform(Point2D ptSrc, Point2D ptDst)
		...
		double tmp_x = (m11 * m22 - m12 * m21) * x + (m02 * m21 - m01 * m22)
				* y + (m01 * m12 - m02 * m11);
		double tmp_y = (m12 * m20 - m10 * m22) * x + (m00 * m22 - m02 * m20)
				* y + (m02 * m10 - m00 * m12);
		double w = (m10 * m21 - m11 * m20) * x + (m01 * m20 - m00 * m21) * y
				+ (m00 * m11 - m01 * m10);
	...
	public void inverseTransform(double[] srcPts, int srcOff, double[] dstPts,
		...
		while (numPts-- > 0) {
			...
			double tmp_x = (m11 * m22 - m12 * m21) * x
					+ (m02 * m21 - m01 * m22) * y + (m01 * m12 - m02 * m11);
			double tmp_y = (m12 * m20 - m10 * m22) * x
					+ (m00 * m22 - m02 * m20) * y + (m02 * m10 - m00 * m12);
			double w = (m10 * m21 - m11 * m20) * x + (m01 * m20 - m00 * m21)
					* y + (m00 * m11 - m01 * m10);
	...
	public boolean isIdentity() {
		return m01 == 0.0 && m02 == 0.0 && m10 == 0.0 && m12 == 0.0
				&& m20 == 0.0 && m21 == 0.0 && m22 != 0.0 && m00 / m22 == 1.0
				&& m11 / m22 == 1.0;
	}
