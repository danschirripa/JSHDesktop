 ...
 */
package jshdesktop.com.pump.job;
...
/**
 * A job that calculates a series of samples. This series is identified by
 * <code>longs</code>: so it can be so large that you should not store the
 * results in memory unless you know an instance of this class is dealing with a
 * limited amount of data.
 * 
 * <p>
 * A <code>WeakReference</code> is kept of this job so it can be retrieved by
 * its ID (until garbage collection consumes it).
 * 
 * @param <T>
 */
public abstract class SampleJob<T> {
	 ...
	 * A range of samples.
	 * <p>
	 * This class is a simple encapsulation of an initial sample index and the
	 * number of samples. The initial index is a <code>long</code>, but the
	 * amount is restricted to an <code>int</code> (so we can safely create an
	...
	public final static class Range implements Serializable, Comparable<Range> {
		...
		public Range add(Range r) throws OverflowException,
				NoncontinuousException {
			...
			if ((indexA1 <= indexB1 && indexB1 <= indexA2)
					...
					|| (indexA2 == indexB1 - 1) || (indexB2 == indexA1 - 1)) {
		...
		public Range subtract(Range r) throws NoncontinuousException {
			...
			if (indexA1 <= indexB1 && indexB1 <= indexA2) {
				if (indexA1 <= indexB2 && indexB2 <= indexA2) {
					throw new NoncontinuousException("This range (" + this
							+ ") contains the argument (" + r
							+ "), so subtract is not possible.");
				}
				return new Range(indexA1, (int) (indexB1 - indexA1 + 1));
			} else if (indexA1 <= indexB2 && indexB2 <= indexA2) {
				return new Range(indexB2 + 1,
						(int) (indexA2 - (indexB2 + 1) + 1));
			}
			return this;
		}
	 ...
	 * 
	 * @param id
	 *            a unique identifier for this job. This must be unique for the
	 *            entire session.
	 * @param sampleCount
	 *            the number of samples this job can calculate.
	 */
	 ...
	 * 
	 * <p>
	 * This method may be asked to recreate redundant data. (The
	 * {@link jshdesktop.com.pump.job.SampleJobExecutor} makes sure never to do this, but
	 * there is technically nothing wrong with it.)
	 * 
	 * <p>
	 * This method simply performs a few boundary checks and invokes
	 * {@link #doCalculate(long, long)}
	 * 
	 * @param range
	 *            the range of samples to calculate.
	...
	public final T[] calculate(long sampleIndex, int length) {
		if (sampleIndex < 0)
			throw new IllegalArgumentException("sampleIndex (" + sampleIndex
					+ ") must not be negative");
		if (sampleIndex + length > sampleCount)
			throw new IllegalArgumentException("sampleIndex + length ("
					+ sampleIndex + " + " + length
					+ ") must not exceed sampleCount (" + sampleCount + ")");
	 ...
	 * 
	 * <p>
	 * Subclasses must override this method to generate data. Boundary checking
	 * is provided by the {@link #calculate(long, int)} method.
	 * 
	 * <p>
	 * This method may be asked to recreate redundant data. (The
	 * {@link jshdesktop.com.pump.job.SampleJobExecutor} makes sure never to do this, but
