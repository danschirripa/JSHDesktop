 ...
 */
package jshdesktop.com.pump.awt;
...
import jshdesktop.com.pump.geom.GeneralPathWriter;
import jshdesktop.com.pump.geom.PathSegment;
import jshdesktop.com.pump.geom.PathWriter;
import jshdesktop.com.pump.geom.SimplifiedPathIterator;

/**
 * This creates a calligraphic or ribbon-like effect while tracing a shape.
 * <p>
 * There are two separate shape bodies being written. If you paint them
 * separately: this creates an "over" and "under" effect. If you paint them the
 * same color: then this class is basically a <code>java.awt.Stroke</code>.
 */
public class CalligraphyPathWriter extends PathWriter {
	...
	/**
	 * This is only used internally when we require a PathWriter to write
	 * fragments of paths.
	 */
	 ...
	 * 
	 * @param angle
	 *            the angle of the nib in this path.
	 * @param offset1
	 *            the offset of one side of the traced shape. One side of shape
	 *            you trace will be offset by: <br>
	 *            (+offset1*cos(angle), +offset1*sin(angle) )
	 *            <p>
	 *            It is possible for this value to be zero, which means one side
	 *            of this path will exactly line up with the original shape
	 *            being traced.
	 *            <p>
	 *            If both offsets are equal: then no new path segments will be
	 *            visible because they will be pencil then.
	 *            <p>
	 *            To achieve a simple stroke: one offset should be (+width/2),
	 *            and the other offset should be (-width/2).
	 * @param offset2
	 *            the offset of the other side of the traced shape. This side of
	 *            shape will be offset by: <br>
	 *            (+offset2*cos(angle), +offset2*sin(angle) )
	 * @param destination
	 *            the destination for the data to write.
	 */
	public CalligraphyPathWriter(float angle, float offset1, float offset2,
			GeneralPath destination) {
		this(angle, offset1, offset2, new GeneralPathWriter(destination),
				new GeneralPathWriter(destination));
	}
	 ...
	 *            (+offset2*cos(angle), +offset2*sin(angle) )
	 * @param dest1
	 *            the destination for half of the calligraphic shape.
	 * @param dest2
	 *            the destination for the other half. This may be the same as
	 *            <code>dest1</code> if you plan on painting this path at the
	 *            same time with the same fill.
	 */
	public CalligraphyPathWriter(float angle, float offset1, float offset2,
			PathWriter dest1, PathWriter dest2) {
	...
	public synchronized void lineTo(final float x, final float y) {
		...
		PathSegment.Float moveTo = new PathSegment.Float(
				tail.data[tail.data.length - 2],
				tail.data[tail.data.length - 1]);
	...
	@Override
	public synchronized void quadTo(final float cx, final float cy,
		...
		int simplifiedSegment = SimplifiedPathIterator.simplify(
				PathIterator.SEG_QUADTO, moveTo.data[0], moveTo.data[1],
				quad.data);
		...
		if (t >= 0 && t <= 1) {
			...
			PathWriter.quadTo(tailWriter, 0, t, x_[0], x_[1], x_[2], y_[0],
					y_[1], y_[2]);
			flush(quad.getX(t), quad.getY(t));

			defineTrack(t, 1, quad);
			PathWriter.quadTo(tailWriter, t, 1, x_[0], x_[1], x_[2], y_[0],
					y_[1], y_[2]);
		} else {
	...
	public synchronized void curveTo(float cx1, float cy1, float cx2,
		...
		int simplifiedSegment = SimplifiedPathIterator.simplify(
				PathIterator.SEG_CUBICTO, moveTo.data[0], moveTo.data[1],
				cubic.data);
		if (simplifiedSegment == PathIterator.SEG_LINETO) {
		...
		if (times != null) {
			...
			if (times.length == 2) {
				...
				PathWriter.cubicTo(tailWriter, 0, times[0], x_[0], x_[1],
						x_[2], x_[3], y_[0], y_[1], y_[2], y_[3]);
				flush(cubic.getX(times[0]), cubic.getY(times[0]));
				defineTrack(times[0], times[1], cubic);
				PathWriter.cubicTo(tailWriter, times[0], times[1], x_[0],
						x_[1], x_[2], x_[3], y_[0], y_[1], y_[2], y_[3]);
				flush(cubic.getX(times[1]), cubic.getY(times[1]));
				defineTrack(times[1], 1, cubic);
				PathWriter.cubicTo(tailWriter, times[1], 1, x_[0], x_[1],
						x_[2], x_[3], y_[0], y_[1], y_[2], y_[3]);
			} else {
				defineTrack(0, times[0], cubic);
				PathWriter.cubicTo(tailWriter, 0, times[0], x_[0], x_[1],
				...
				PathWriter.cubicTo(tailWriter, times[0], 1, x_[0], x_[1],
						x_[2], x_[3], y_[0], y_[1], y_[2], y_[3]);
			}
			return;
		}
	...
	public synchronized void closePath() {
		...
		if (Math.abs(lastX - moveX) > .00001
				|| Math.abs(lastY - moveY) > .00001)
			lineTo(moveX, moveY);
	...
	public synchronized void flush() {
		try {
			...
			/**
			 * This occurs if a moveTo is followed by a close with no segment
			 * data.
			 */
			if (writingTrack1 == null)
				return;
			 ...
			 * problems when dest1 and dest2 are the same path. The same basic
			 * path data is being written either way.
			 */
			if (writingTrack1.booleanValue()) {
				dest = dest1;
				transform1 = AffineTransform.getTranslateInstance(
						offset1 * cos, offset1 * sin);
				transform2 = AffineTransform.getTranslateInstance(
						offset2 * cos, offset2 * sin);
			} else {
				dest = dest2;
				transform1 = AffineTransform.getTranslateInstance(
						offset2 * cos, offset2 * sin);
			...
			dest.lineTo((float) pts[pts.length - 2],
					(float) pts[pts.length - 1]);
