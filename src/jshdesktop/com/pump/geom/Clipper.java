...
public abstract class Clipper {
	...
	static class ClippedPath {
		...
		public void curveTo(Function xf, Function yf, double t0, double t1) {
			...
			g.curveTo((float) (x0 + dx0 / 3), (float) (y0 + dy0 / 3),
					(float) (x1 - dx1 / 3), (float) (y1 - dy1 / 3),
					(float) (x1), (float) (y1));
		}
	 ...
	 * @return a <code>GeneralPath</code> enclosing the new shape.
	 */
	public static GeneralPath clipToRect(Shape s, AffineTransform t,
			Rectangle2D r) {
	...
	/**
	 * Calculates the t-values for which this shape intersects the parametric
	 * function provided. The values in the array are not expected to be sorted.
	 * 
	 * @param xf
	 *            the x parametric curve
	 * @param yf
	 *            the y parametric curve
				...
				for (int a = 0; a < tCtr; a++) {
					if (a > 0 && intersectionTimes[a] == intersectionTimes[a
						...
						if ((xf instanceof LFunction) || thisValueIsCapped
							...
							p.curveTo(xf, yf, intersectionTimes[a - 1],
									intersectionTimes[a]);
						} else {
	...
	/**
	 * By default if a Graphics2D is asked to clip to a new shape, it may resort
	 ...
	 * <P>
	 * This should only be used to replace <code>Graphics2D.clip()</code>, not
	 * <code>Graphics2D.setClip()</code>.
	 * 
	 * @param g
	 *            the graphics2D to clip to
	 * @param newClip
	 *            the new clip
	 */
	public static void clip(Graphics2D g, Shape newClip) {
		...
		if (oldRect != null && newRect != null) {
			...
			if (intersectedClip.getWidth() < 0
					|| intersectedClip.getHeight() < 0) {
				// a negative width or height indicates there's no real
				// intersection
				intersectedClip.setFrame(intersectedClip.getX(),
						intersectedClip.getY(), 0, 0);
			}
