 ...
 * <P>
 * With this abstract class you could filter shape data as it is being written.
 * The simplest extension of this is the {@link GeneralPathWriter}.
 * 
 * <P>
 * This is designed to match the <code>GeneralPath</code> method signatures.
 * 
 */
public abstract class PathWriter {
	 ...
	 *            the y-coordinate of the end point.
	 * */
	public abstract void curveTo(float cx1, float cy1, float cx2, float cy2,
			float x, float y);
	 ...
	 * information.
	 * <P>
	 * For example, some writers may need to perform complex calculations and
	 * re-organize shapes. Or some writers may simply be buffering path
	 * instructions to better manage memory. Calling this method guarantees that
	 * all path instructions be immediately processed.
	 * <P>
	 * Because some writers may be manipulating path data you should <i>not call
	 * this method</i> until all shape data has been written.
	 * 
	 */
	 ...
	 * Writes a shape.
	 * <P>
	 * This iterates through the shape and makes the appropriate calls to
	 * <code>moveTo()</code>, <code>curveTo()</code>, etc.
	 * 
	 * @param s
	 *            the shape to write.
	 */
	 ...
	 * Writes a path.
	 * <P>
	 * This iterates through the path and makes the appropriate calls to
	 * <code>moveTo()</code>, <code>curveTo()</code>, etc.
	 * 
	 * @param i
	 *            the path to write.
	...
	/**
	 * This appends a cubic curve to the PathWriter provided that ranges from t
	 * = t0 to t = t1. This assumes that the path already ends at the point
	 * (ax*t0*t0*t0+bx*t0*t0+cx*t0+dx, ay*t0*t0*t0+by*t0*t0+cy*t0+dx).
	 * 
	 * @param path
	 ...
	 *            the constant for the y parametric equation.
	 */
	public static void cubicTo(PathWriter path, double t0, double t1,
			double ax, double bx, double cx, double dx, double ay, double by,
			double cy, double dy) {
		cubicTo2(path, t0, t1, ax, bx, cx, dx, ay, by, cy, dy);
	}

	private static void cubicTo2(Object obj, double t0, double t1, double ax,
			double bx, double cx, double dx, double ay, double by, double cy,
			double dy) {
		...
		if (obj instanceof GeneralPath) {
			((GeneralPath) obj).curveTo((float) cx0, (float) cy0, (float) cx1,
	...
	/**
	 * This appends a quadratic curve to the PathWriter provided that ranges
	 * from t = t0 to t = t1. This assumes that the path already ends at the
	 * point (ax*t0*t0+bx*t0+cx, ay*t0*t0+by*t0+cy).
	 * 
	 * @param path
	 ...
	 *            the constant for the y parametric equation.
	 */
	public static void quadTo(PathWriter path, double t0, double t1, double ax,
			double bx, double cx, double ay, double by, double cy) {
		quadTo2(path, t0, t1, ax, bx, cx, ay, by, cy);
	}

	private static void quadTo2(Object obj, double t0, double t1, double ax,
			double bx, double cx, double ay, double by, double cy) {
		double tZ = (t0 + t1) / 2.0;
		...
		if (obj instanceof GeneralPath) {
			...
			((PathWriter) obj).quadTo((float) ctrlX, (float) ctrlY, (float) x1,
					(float) y1);
		}
	}
}