 ...
 */
package jshdesktop.com.pump.geom.intersect;

/**
 * This {@link IntersectionIdentifier} uses a 2D binary search to hone in on
 * potential intersections.
 * <P>
 * It breaks curves up according to their critical points, so each curve segment
 * is guaranteed to only move in 1 general direction between any two give t
 * values.
 * 
 * TODO warning: this project is buggy. The CurveIntersectionApp can be used to
 * highlight several instances where two cubic curves do not produce
 * intersections (even though the human eye can tell they should). Also
 * sometimes exceptions are thrown. The simplest starting point for these issues
 * is: Curve 1: m 100.0 100.0 c 125.0 125.0 125.0 275.0 100.0 300.0 Curve 2: m
 * 113.0 120.0 c 88.0 145.0 88.0 295.0 113.0 320.0
 */
public class BinarySearchIntersectionIdentifier extends IntersectionIdentifier {
	 ...
	 * of 4 items (optionally including one and zero).
	 * 
	 * @param dest
	 *            the array the list is stored in.
	 * @param includeOneAndZero
	 *            whether the list should include, and be confined to, values
	 *            within [0, 1]
	 * @return the number of items in this list.
	 */
	private static int getQuadraticTimes(double[] dest, double ax, double bx,
			double cx, double ay, double by, double cy,
			boolean includeOneAndZero) {
		int size = 0;
	...
	/**
	 * Add an element to a sorted list, returning false if the element already
	 * exists.
	 */
	private static boolean add(double[] dest, double value, int pos) {
		/*
		 * We could implement a binary search here to identify the exact
		 * position, but this method is only ever used for lists with at most 6
		 * elements, so I'm not sure it's worth it here.
		 * 
		 * (Don't lecture me about big-O notations: 6 elements means this method
		 * might involve 6 comparisons. A binary search would 3 divisions and 3
		 * comparisons; surely a division is more expensive than a comparison?)
		 * 
		 * If this is abstracted to a public static method for general use it
		 * should definitely be converted to a binary search.
		 */
		for (int a = 0; a < pos; a++) {
			if (dest[a] == value)
				return false;
	 ...
	 * items (optionally including one and zero).
	 * 
	 * @param dest
	 *            the array the list is stored in.
	 * @param includeOneAndZero
	 ...
	 * @return the number of items in this list.
	 */
	private static int getCubicTimes(double[] dest, double ax, double bx,
			double cx, double dx, double ay, double by, double cy, double dy,
			boolean includeOneAndZero) {
		int size = 0;
		if (includeOneAndZero) {
			size = 2;
	 ...
	 * as zero. (And likewise lines can treat the "a" and "b" terms as zero.)
	 * 
	 * @param ax1
	 ...
	 * @param returnType
	 *            which values to store in the destination array.
	 * @return whether the two curves intersect.
	 */
	private static boolean binarySearch(double[] dest, int arrayOffset,
			double ax1, double bx1, double cx1, double dx1, double ay1,
			double by1, double cy1, double dy1, double startT1, double endT1,
			double ax2, double bx2, double cx2, double dx2, double ay2,
			double by2, double cy2, double dy2, double startT2, double endT2,
			Return returnType) {
		...
		double startX2 = ((ax2 * startT2 + bx2) * startT2 + cx2) * startT2
				+ dx2;
		double startY2 = ((ay2 * startT2 + by2) * startT2 + cy2) * startT2
				+ dy2;
		double endX2 = ((ax2 * endT2 + bx2) * endT2 + cx2) * endT2 + dx2;
		double endY2 = ((ay2 * endT2 + by2) * endT2 + cy2) * endT2 + dy2;

		boolean intersects = intersects(startX1, startY1, endX1, endY1,
		...
		while (true) {
			...
			if ((!split1) && (!split2)) {
				...
				case X_Y:
					dest[arrayOffset] = ((ax1 * t1 + bx1) * t1 + cx1) * t1
							+ dx1;
					dest[arrayOffset + 1] = ((ay1 * t1 + by1) * t1 + cy1) * t1
							+ dy1;
					break;
				}
				return true;
				...
				double midX2 = ((ax2 * midT2 + bx2) * midT2 + cx2) * midT2
						+ dx2;
				double midY2 = ((ay2 * midT2 + by2) * midT2 + cy2) * midT2
						+ dy2;

				boolean intersects1 = intersects(startX1, startY1, midX1,
						midY1, startX2, startY2, midX2, midY2);
				boolean intersects2 = intersects(midX1, midY1, endX1, endY1,
						startX2, startY2, midX2, midY2);
				boolean intersects3 = intersects(startX1, startY1, midX1,
						midY1, midX2, midY2, endX2, endY2);
				boolean intersects4 = intersects(midX1, midY1, endX1, endY1,
						midX2, midY2, endX2, endY2);
				...
				if ((intersects1) && (!intersects2) && (!intersects3)
					...
					if (intersects1
							&& binarySearch(dest, arrayOffset, ax1, bx1, cx1,
									dx1, ay1, by1, cy1, dy1, startT1, midT1,
									ax2, bx2, cx2, dx2, ay2, by2, cy2, dy2,
									startT2, midT2, returnType))
						return true;

					if (intersects2
							&& binarySearch(dest, arrayOffset, ax1, bx1, cx1,
									dx1, ay1, by1, cy1, dy1, midT1, endT1, ax2,
									bx2, cx2, dx2, ay2, by2, cy2, dy2, startT2,
									midT2, returnType))
						return true;
					...
					if (intersects3
							&& binarySearch(dest, arrayOffset, ax1, bx1, cx1,
									dx1, ay1, by1, cy1, dy1, startT1, midT1,
									ax2, bx2, cx2, dx2, ay2, by2, cy2, dy2,
									midT2, endT2, returnType))
						return true;

					if (intersects4
							&& binarySearch(dest, arrayOffset, ax1, bx1, cx1,
									dx1, ay1, by1, cy1, dy1, midT1, endT1, ax2,
									bx2, cx2, dx2, ay2, by2, cy2, dy2, midT2,
									endT2, returnType))
						return true;
				...
				boolean intersects1 = intersects(startX1, startY1, midX1,
						midY1, startX2, startY2, endX2, endY2);
				boolean intersects2 = intersects(midX1, midY1, endX1, endY1,
						startX2, startY2, endX2, endY2);
				...
				boolean intersects1 = intersects(startX1, startY1, endX1,
						endY1, startX2, startY2, midX2, midY2);
				boolean intersects2 = intersects(startX1, startY1, endX1,
						endY1, midX2, midY2, endX2, endY2);
				...
				if ((intersects1) && (!intersects2)) {
					...
					if (intersects1
							&& binarySearch(dest, arrayOffset, ax1, bx1, cx1,
									dx1, ay1, by1, cy1, dy1, startT1, endT1,
									ax2, bx2, cx2, dx2, ay2, by2, cy2, dy2,
									startT2, midT2, returnType))
						return true;
	...
	public int lineQuadratic(double ax1, double bx1, double ay1, double by1,
		...
		int size1 = 2;
		int size2 = getQuadraticTimes(array2, ax2, bx2, cx2, ay2, by2, cy2,
				true);
		...
		for (int i1 = 0; i1 < size1 - 1; i1++) {
			for (int i2 = 0; i2 < size2 - 1; i2++) {
				if (binarySearch(dest, offset + returnTypeLength * hits, 0, 0,
						ax1, bx1, 0, 0, ay1, by1, array1[i1], array1[i1 + 1],
						0, ax2, bx2, cx2, 0, ay2, by2, cy2, array2[i2],
						array2[i2 + 1], returnType))
					hits++;
			}
		}
		return hits;
	...
	public int lineCubic(double ax1, double bx1, double ay1, double by1,
		...
		int size1 = 2;
		int size2 = getCubicTimes(array2, ax2, bx2, cx2, dx2, ay2, by2, cy2,
				dy2, true);
		...
		for (int i1 = 0; i1 < size1 - 1; i1++) {
			for (int i2 = 0; i2 < size2 - 1; i2++) {
				if (binarySearch(dest, offset + returnTypeLength * hits, 0, 0,
						ax1, bx1, 0, 0, ay1, by1, array1[i1], array1[i1 + 1],
						ax2, bx2, cx2, dx2, ay2, by2, cy2, dy2, array2[i2],
						array2[i2 + 1], returnType))
					hits++;
			}
		}
	...
	public int quadraticQuadratic(double ax1, double bx1, double cx1,
		...
		int size1 = getQuadraticTimes(array1, ax1, bx1, cx1, ay1, by1, cy1,
				true);
		int size2 = getQuadraticTimes(array2, ax2, bx2, cx2, ay2, by2, cy2,
				true);
		...
		for (int i1 = 0; i1 < size1 - 1; i1++) {
			for (int i2 = 0; i2 < size2 - 1; i2++) {
				if (binarySearch(dest, offset + returnTypeLength * hits, 0,
						ax1, bx1, cx1, 0, ay1, by1, cy1, array1[i1],
						array1[i1 + 1], 0, ax2, bx2, cx2, 0, ay2, by2, cy2,
						array2[i2], array2[i2 + 1], returnType))
					hits++;
			}
		}
		return hits;
	...
	public int quadraticCubic(double ax1, double bx1, double cx1, double ay1,
		...
		for (int i1 = 0; i1 < size1 - 1; i1++) {
			for (int i2 = 0; i2 < size2 - 1; i2++) {
				if (binarySearch(dest, offset + returnTypeLength * hits, 0,
						ax1, bx1, cx1, 0, ay1, by1, cy1, array1[i1],
						array1[i1 + 1], ax2, bx2, cx2, dx2, ay2, by2, cy2, dy2,
						array2[i2], array2[i2 + 1], returnType))
					hits++;
			}
		}
	...
	public int cubicCubic(double ax1, double bx1, double cx1, double dx1,
		...
		int size1 = getCubicTimes(array1, ax1, bx1, cx1, dx1, ay1, by1, cy1,
				dy2, true);
		int size2 = getCubicTimes(array2, ax2, bx2, cx2, dx2, ay2, by2, cy2,
				dy2, true);
		...
		for (int i1 = 0; i1 < size1 - 1; i1++) {
			for (int i2 = 0; i2 < size2 - 1; i2++) {
				if (binarySearch(dest, offset + returnTypeLength * hits, ax1,
						bx1, cx1, dx1, ay1, by1, cy1, dy1, array1[i1],
						array1[i1 + 1], ax2, bx2, cx2, dx2, ay2, by2, cy2, dy2,
						array2[i2], array2[i2 + 1], returnType))
					hits++;
			}
