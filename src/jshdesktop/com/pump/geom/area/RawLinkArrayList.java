...
public class RawLinkArrayList {
	 ...
	 */
	protected RawLinkArrayList(int initialCapacity) {
		if (initialCapacity == 0) {
			this.elementData = EMPTY;
		} else {
			this.elementData = new CurveLinkX[initialCapacity];
		}
	}
	...
	protected void ensureCapacity(int minCapacity) {
		...
		if (minCapacity > oldCapacity) {
			...
			elementData = new CurveLinkX[newCapacity];
	 ...
	 */
	protected void add(CurveLinkX o) {
		ensureCapacity(elementCount + 1);
		elementData[elementCount++] = o;
	}

	protected CurveLinkX[] getArray() {
		return elementData;
	}

	protected CurveLinkX[] getArray(int minArraySize) {
