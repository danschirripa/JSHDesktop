...
import jshdesktop.com.pump.geom.IntersectionsTest.MachineErrorException;
...
public class RelationshipTest extends BasicTestElement {
	...
	private AreaX getShape(int i) {
		...
		for (int a = 0; a < 3; a++) {
			if (i % 2 == 0) { // small
				if (a == 0) {
					path.moveTo((float) (40 + random.nextDouble() * 20),
							(float) (40 + random.nextDouble() * 20));
				} else {
					if (type.equals("Lines")) {
						path.lineTo((float) (40 + random.nextDouble() * 20),
								(float) (40 + random.nextDouble() * 20));
					} else if (type.equals("Quads")) {
						path.quadTo((float) (40 + random.nextDouble() * 20),
								(float) (40 + random.nextDouble() * 20),
								(float) (40 + random.nextDouble() * 20),
								(float) (40 + random.nextDouble() * 20));
					} else if (type.equals("Cubics")) {
						path.curveTo((float) (40 + random.nextDouble() * 20),
								(float) (40 + random.nextDouble() * 20),
								(float) (40 + random.nextDouble() * 20),
								(float) (40 + random.nextDouble() * 20),
								(float) (40 + random.nextDouble() * 20),
				...
				if (a == 0) {
					path.moveTo((float) (random.nextDouble() * 100),
							(float) (random.nextDouble() * 100));
				} else {
					if (type.equals("Lines")) {
						path.lineTo((float) (random.nextDouble() * 100),
								(float) (random.nextDouble() * 100));
					} else if (type.equals("Quads")) {
						path.quadTo((float) (random.nextDouble() * 100),
								(float) (random.nextDouble() * 100),
								(float) (random.nextDouble() * 100),
								(float) (random.nextDouble() * 100));
					} else if (type.equals("Cubics")) {
						path.curveTo((float) (random.nextDouble() * 100),
								(float) (random.nextDouble() * 100),
								(float) (random.nextDouble() * 100),
								(float) (random.nextDouble() * 100),
								(float) (random.nextDouble() * 100),
	...
	public void doTest() {
		...
		for (int a = 0; a < testCount; a++) {
			...
			for (int b = 0; b < testCount; b++) {
				...
				Rectangle2D r;
				if (boundsA.getWidth() * boundsA.getHeight() < boundsB
						.getWidth() * boundsB.getHeight()) {
					r = boundsA;
				} else {
					r = boundsB;
				}
				...
				boolean intersects;
				if (k == null) {
					intersects = isIntersectionObserved(shapeA, shapeB, image1,
							image2, r, 1);
				...
				if (intersects == false) {
					...
					if (a == b) {
						...
						if (value == null) {
							...
							if (value.equals("r")
									&& rel == AreaX.RELATIONSHIP_RHS_CONTAINS) {
								passed = true;
							} else if (value.equals("l")
									&& rel == AreaX.RELATIONSHIP_LHS_CONTAINS) {
								passed = true;
							} else if (value.equals("c")
									&& rel == AreaX.RELATIONSHIP_COMPLEX) {
								passed = true;
							} else {
								passed = false;
							}
	...
	public static final void fill(Shape shape, AffineTransform at,
			BufferedImage bi, boolean shapeA, boolean includeStroke) {
		clear(bi);
		...
		if (includeStroke) {
			// add a little padding. Since we keep zooming in to verify
			// intersection, this should only help us:
