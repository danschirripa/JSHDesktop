...
/**
 * This filters a <code>PathIterator</code> and makes sure that each curve is of
 * the smallest possible degree.
 * <P>
 * In addition to being more efficient, this can avoid divide-by-zero errors for
 * some operations.
 * 
 */
public class SimplifiedPathIterator implements PathIterator {
	/**
	 * This is the tolerance a term can be to be considered "zero". This value
	 * is .0001.
	 */
	...
	/**
	 * Returns true if all 3 points are on the same line. This checks to see if: <BR>
	 * <code>x1*(y2-y3)+x2*(y3-y1)+x3*(y1-y2)&lt;TOL*TOL</code>
	 * 
	 * @param x1
	 ...
	 */
	public static boolean collinear(double x1, double y1, double x2, double y2,
			double x3, double y3) {
	 ...
	 * 
	 * @param type
	 *            the current expected type of the segment data
	 * @param lastX
	 *            the previous X value from which this segment begins
	 * @param lastY
	 *            the previous Y value from which this segment begins
	 * @param data
	 *            the data of the current segment.
	 * @return the new segment type, or the original <code>type</code> argument
	 ...
	 *         if nothing was modified.
	 */
	public static int simplify(int type, double lastX, double lastY,
			double[] data) {
		if (type == SEG_CUBICTO) {
			if (collinear(lastX, lastY, data[4], data[5], data[0], data[1])
					&& collinear(lastX, lastY, data[4], data[5], data[2],
							data[3])) {
				data[0] = data[4];
				data[1] = data[5];
				return SEG_LINETO;
