...
/**
 * In this transition one image breaks into several smaller tiles and then is
 * whisked away as if by a wind. Or, seen backwards, a flurry of incoming tiles
 * assemble into the incoming image.
 * <p>
 * Here are playback samples:
 * <p>
 * <table summary="Sample Animations of FlurryTransition2D" cellspacing="50" border="0">
 * <tr>
 * <td align="center">
 * <img src=
...
public class FlurryTransition2D extends Transition2D {
	...
	public static Transition[] getDemoTransitions() {
		return new Transition[] { new FlurryTransition2D(IN),
	...
	public FlurryTransition2D(int type) {
		if (!(type == OUT || type == IN)) {
			throw new IllegalArgumentException(
					"This transition must use OUT or IN.");
		}
		this.type = type;
	}
	...
	public Transition2DInstruction[] getInstructions(float progress,
		...
		for (int a = 0; a < v1.size(); a++) {
			...
			random.setSeed(a);
			Shape clipping = r;
			Point2D center = new Point2D.Double(r.getCenterX() - 200f / 2f,
					r.getCenterY() - 200f / 2f);
			float k = (float) (Math.sqrt(center.getX() * center.getX()
					+ center.getY() * center.getY()) / Math.sqrt(200f * 200f
					/ 4 + 200f * 200f / 4));
			k = (1 - progress) * k + progress;
			float scaleProgress = (float) Math.pow(2 * progress * k,
					.02 + 4 * random.nextFloat());
			...
			transform.concatenate(TransformUtils.createAffineTransform(0, 0, 0,
					200, 200, 0, 0, 0, 0, size.height, size.width, 0));
			...
			transform = TransformUtils.createAffineTransform(r.getX()
					* size.width / 200f, r.getY() * size.height / 200f,
					(r.getX() + r.getWidth()) * size.width / 200f, r.getY()
							* size.height / 200f, r.getX() * size.width / 200f,
					(r.getY() + r.getHeight()) * size.height / 200f,
