...
import jshdesktop.com.pump.io.GuardedInputStream;
import jshdesktop.com.pump.io.IOUtils;
import jshdesktop.com.pump.io.MeasuredInputStream;
import jshdesktop.com.pump.util.Receiver;
import jshdesktop.org.apache.tools.tar.TarEntry;
import jshdesktop.org.apache.tools.tar.TarInputStream;
...
public class TarArchiveLocation extends ArchiveLocation<TarEntry> {
	...
	@Override
	protected void listArchiveEntries(Receiver<TarEntry> receiver)
			throws IOException {
		...
		try {
			...
			try {
				...
				while (entry != null) {
					...
					EntryLocation loc = new EntryLocation(entryStartPtr,
							entryLength);
					archiveEntryToLocation.put(entry.getName(), loc);
					receiver.add(entry);

					// All the data of a TarEntry is expressed in blocks of 512
					// bytes, rounded up:
	...
	@Override
	protected ArchiveLocation<TarEntry> createArchiveLocation(
			ArchiveLocation<TarEntry> root, TarEntry t) {
