 ...
 */
package jshdesktop.com.pump.io.location;
...
public class LocationFactory {
	...
	protected SuffixIOLocationFilter zipFilter = new SuffixIOLocationFilter(
			false, false, true, "jar", "zip");
	protected SuffixIOLocationFilter tarFilter = new SuffixIOLocationFilter(
			false, false, true, "tar");
	...
	/**
	 * Create an <code>IOLocation</code> based on a <code>File</code>. This
	 * factory may not return a <code>FileLocation</code>. (For example: it
	 * might return a <code>ZipArchiveLocation</code>, or a special subclass of
	...
	/**
	 * Create a <code>FileLocation</code> based on a <code>File</code>. This is
	 * not recommended unless you explicitly need a <code>FileLocation</code>.
	 * 
	 * @param file
	...
	/**
	 * Create a <code>URLLocation</code> based on a <code>URL</code>. This is
	 * not recommended unless you explicitly need a <code>URLLocation</code>.
	 * 
	 * @param url
	...
	/**
	 * @return an <code>IOLocation</code> based on a <code>URL</code>. This
	 *         factory may not return a <code>URLLocation</code>. (For example:
	 *         it might return a <code>FileLocation</code>, a
	 *         <code>ZipArchiveLocation</code>, or a special subclass of
	 *         <code>URLLocation</code>.)
	 * @param url
	 *            the url this is based on.
	 */
	...
	protected IOLocation filter(IOLocation loc) {
		...
		/*
		 * TODO: consider making public, and let users navigate zips inside
		 * zips?
		 */
		// TODO: test queries/fragments/etc
		if (zipFilter.filter(loc) != null
				&& (!(loc instanceof ZipArchiveLocation))) {
			return filter(new ZipArchiveLocation(loc));
		} else if (tarFilter.filter(loc) != null
				&& (!(loc instanceof TarArchiveLocation))) {
