...
import jshdesktop.com.pump.awt.TextSize;
import jshdesktop.com.pump.swing.DialogFooter;
import jshdesktop.com.pump.swing.QOptionPane;

public class BasicQOptionPaneUI extends QOptionPaneUI {

	private static Color readColor(ResourceBundle bundle, String key,
			Color defaultColor) {
		try {
	...
	private static boolean readBoolean(ResourceBundle bundle, String key,
	...
	private static int readInt(ResourceBundle bundle, String key,
			int defaultValue) {
		try {
			String string = bundle.getString(key);
			if (string != null) {
	...
	private static Font readFont(ResourceBundle bundle, String key,
			Font defaultFont) {
		...
		Font font = null;
		if (baseFont != null && baseFont.length() > 2
				&& baseFont.charAt(0) == '"'
				&& baseFont.charAt(baseFont.length() - 1) == '"') {
			baseFont = baseFont.substring(1, baseFont.length() - 1);
			font = UIManager.getFont(baseFont);
		} else if (baseFont != null) {
			font = new Font(baseFont, defaultFont.getStyle(),
					defaultFont.getSize());
		} else {
			font = defaultFont;
		}
	...
	private static ResourceBundle getDefaultResourceBundle() {
		...
		// this guarantees the JarWriter will sniff this out and include all the
		// appropriate files
		// TODO: revisit this
	...
	private PropertyChangeListener customizationListener = new PropertyChangeListener() {
		public void propertyChange(PropertyChangeEvent evt) {
			...
			if (type.isInstance(evt.getNewValue()) == false) {
				String desc = evt.getNewValue() == null ? "null" : evt
						.getNewValue().getClass().getName();
				throw new RuntimeException("the key \"" + evt.getPropertyName()
						+ "\" should map to a " + type.getName() + " (found "
						+ desc + ")");
			}
	...
	protected void updateFooter(QOptionPane pane) {
		...
		if (footer != null) {
			...
			boolean leftAlign = getBoolean(pane,
					KEY_FOOTER_LEFT_ALIGN_UNSAFE_BUTTONS);
