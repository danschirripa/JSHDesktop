...
import jshdesktop.com.pump.swing.io.LocationPaneDialog;

public abstract class BasicSaveLocationPaneUI extends SaveLocationPaneUI {
	public static final Icon DOWN_ICON = new TriangleIcon(SwingConstants.SOUTH,
			10, 5);
	public static final Icon UP_ICON = new TriangleIcon(SwingConstants.NORTH,
			10, 5);
	...
	public BasicSaveLocationPaneUI(LocationPane locPane) {
		...
		footer = new DialogFooter(getLeftFooterControls(), new JComponent[] {
				commitButton, cancelButton }, false, commitButton);

		saveField.getDocument().addDocumentListener(new DocumentListener() {

			public void changedUpdate(DocumentEvent e) {
				// can't run this now: modifying a document during a document
				// event throws exceptions
		...
		locationPane.addPropertyChangeListener("suffix",
				new PropertyChangeListener() {
					public void propertyChange(PropertyChangeEvent evt) {
						SwingUtilities.invokeLater(updateTextRunnable);
					}
				});
		locationPane.addPropertyChangeListener(LocationPane.ACCESSORY_KEY,
				new PropertyChangeListener() {
					public void propertyChange(PropertyChangeEvent evt) {
	 ...
	 * <p>
	 * By default this returns the <code>newFolderButton</code>.
	 */
	...
	public void setExpanded(boolean b) {
		...
		Preferences.userNodeForPackage(BasicSaveLocationPaneUI.class)
				.putBoolean(expandedKey, b);
		...
		if (w instanceof LocationPaneDialog) {
			...
			w.setLocation(w.getX() + p.x - newPoint.x, w.getY() + p.y
					- newPoint.y);
		}
	}
	...
	private void updateAccessoryPanel() {
		...
		Component oldAccessory = accessoryDecoration.getComponentCount() > 0 ? accessoryDecoration
				.getComponent(0) : null;
	...
	/**
	 * This fetches the current file name and updates the <code>saveField</code>
	 * text if necessary to conform to the currently assigned suffix.
	 */
	@Override
	...
	/**
	 * Returns the suffix of a string, or null if no suffix is detected.
	 * 
	 * @param s
	 *            a string like "name.foo"
	 * @return the text that follows the last "." in the argument, or null if no
	 *         "." is present.
	 */
	...
	/**
	 * Updates the enabled state of the commit button. By default this method
	 * disables the commit button if the <code>saveField</code> is empty.
	 */
