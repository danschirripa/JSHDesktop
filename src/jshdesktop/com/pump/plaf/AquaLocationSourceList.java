...
public class AquaLocationSourceList extends JTree {
	...
	ChangeListener directoryStackListener = new ChangeListener() {
		public void stateChanged(ChangeEvent e) {
			...
			DefaultMutableTreeNode node = findNode((TreeNode) getModel()
					.getRoot(), loc);
			if (node == null) {
	...
	/**
	 * A <code>DefaultMutableTreeNode</code> that maps to a
	 * <code>IOLocation</code>.
	 */
	class LocationNode extends DefaultMutableTreeNode {
		...
		public void setIcon(Icon i) {
			...
			Rectangle bounds = getUI().getPathBounds(
					AquaLocationSourceList.this, path);
			if (bounds != null)
				repaint(bounds);
		}
	...
	public void add(IOLocation[] locations) {
		for (int a = 0; a < locations.length; a++) {
			...
			if (isDevice) {
				treeModel.insertNodeInto(newNode, devices,
						devices.getChildCount());
			} else {
				treeModel.insertNodeInto(newNode, places,
						places.getChildCount());
			}
		}
	}
