 ...
 */
package jshdesktop.com.pump.plaf;
...
import jshdesktop.com.pump.icon.ArrowIcon;
import jshdesktop.com.pump.icon.IconUtils;
import jshdesktop.com.pump.icon.TriangleIcon;
import jshdesktop.com.pump.io.location.FileLocation;
import jshdesktop.com.pump.io.location.IOLocation;
import jshdesktop.com.pump.io.location.LocationFactory;
import jshdesktop.com.pump.io.location.SearchParameters;
import jshdesktop.com.pump.io.location.SearchResults;
import jshdesktop.com.pump.swing.JThrobber;
import jshdesktop.com.pump.swing.io.LocationBrowser;
import jshdesktop.com.pump.swing.io.LocationHistory;
...
public abstract class LocationPaneUI extends ComponentUI {
	...
	private DocumentListener searchFieldListener = new DocumentListener() {
		...
		public void changedUpdate(DocumentEvent e) {
			...
			if (loc instanceof SearchResults) {
				loc = ((SearchResults) loc).getSearchDirectory();
				// locationPane.getLocationHistory().replace(
				// loc.search(parameters) );
			} else {
				// locationPane.getLocationHistory().append(
				// loc.search(parameters) );
			}
	...
	public LocationPaneUI(LocationPane locationPane) {
		...
		locationPane.getLocationHistory().addChangeListener(
				new ChangeListener() {
					Runnable updateText = new Runnable() {
						public void run() {
							IOLocation current = LocationPaneUI.this.locationPane
									.getLocationHistory().getLocation();
							String newText;
							if (!(current instanceof SearchResults)) {
								newText = "";
							} else {
								// if this SearchResults object was from the
								// result
		...
		comboBox.setRenderer(new IOLabelCellRenderer(comboBox, browser
				.getGraphicCache()));
		...
		commitButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (commitButtonPressed()) {
					...
					Boolean b = (Boolean) LocationPaneUI.this.locationPane
							.getClientProperty("saveLocation.commitButton.dismissWindow");
					if (b == null)
						b = Boolean.TRUE;
		...
		cancelButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (cancelButtonPressed()) {
					...
					Boolean b = (Boolean) LocationPaneUI.this.locationPane
							.getClientProperty("saveLocation.cancelButton.dismissWindow");
					if (b == null)
						b = Boolean.TRUE;
					if (b.booleanValue() == false)
						return;
		...
		locationPane.addHierarchyListener(new HierarchyListener() {
			...
			Runnable updateRunnable = new Runnable() {
				public void run() {
					boolean isShowing = LocationPaneUI.this.locationPane
							.isShowing();
	...
	protected KeyStroke desktopKeystroke = KeyStroke
			.getKeyStroke(KeyEvent.VK_D, Toolkit.getDefaultToolkit()
					.getMenuShortcutKeyMask());
	protected AbstractAction navigateToDesktop = new AbstractAction() {
		private static final long serialVersionUID = 1L;

		public void actionPerformed(ActionEvent e) {
			File desktopFile = new File(System.getProperty("user.dir")
					+ File.separator + "Desktop");
			if (desktopFile.exists()) {
				LocationHistory history = LocationPaneUI.this.locationPane
						.getLocationHistory();
				IOLocation[] list = history.getList();
				for (int a = 0; a < list.length; a++) {
					if (list[a] instanceof FileLocation) {
						IOLocation newLoc = LocationFactory.get().create(
								desktopFile);
	...
	/**
	 * Turn on/off special shortcuts. For example: on Mac command+D should
	 * navigate to the desktop.
	 */
	protected void setShortcutsActive(boolean b) {
		Window window = SwingUtilities.getWindowAncestor(locationPane);
		/**
		 * T4L Bug 21770 had to do with a normally hidden LocationPaneUI
		 * consuming cmd+D keystrokes. So now we only install these keystrokes
		 * if we're visible and in a dialog...
		 */
		if (window instanceof RootPaneContainer && window instanceof Dialog) {
			...
			if (b) {
				rootPane.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(
						desktopKeystroke, "navigateToDesktop");
				rootPane.getInputMap(
						JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(
						desktopKeystroke, "navigateToDesktop");
				rootPane.getActionMap().put("navigateToDesktop",
						navigateToDesktop);
			} else {
				rootPane.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).remove(
						desktopKeystroke);
				rootPane.getInputMap(
						JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).remove(
						desktopKeystroke);
			}
		}
	}
	 ...
	 * be notified when the commit button is clicked, but some UI's might also
	 * check to see if the current file name replaces an existing file and
	 * prompt the user according.
	 */
	...
	public void addCommitListener(ActionListener l) {
		if (commitListeners.contains(l))
			return;
		commitListeners.add(l);
	}

	/**
	 * This is called automatically when <code>commitButtonPressed()</code>
	...
	protected void fireCommitListeners() {
		for (int a = 0; a < commitListeners.size(); a++) {
			...
			try {
				l.actionPerformed(new ActionEvent(commitButton,
						ActionEvent.ACTION_PERFORMED, "commit"));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	 ...
	 * 
	 * @return true this dialog should close normally. <code>false</code>
	 *         indicates that the dialog should not close, and presumably some
	 *         other action is being taken.
	 *         <p>
	 *         For example, in a save pane if a file with this name is detected:
	 *         then a subclass might prompt the user with an error dialog and
	...
	protected boolean commitButtonPressed() {
		if (locationPane instanceof OpenLocationPane) {
			...
			if (olp.isDirectory() == false) {
				IOLocation[] locations = locationPane.getSelectionModel()
						.getSelection();
				if (locations.length == 1 && locations[0].isDirectory()
						&& locations[0].isNavigable()) {
					locationPane.getLocationHistory().append(locations[0]);
					return false;
				}
				// TODO: what happens if you have multiple directories
				// selected and you click "Open", when directories aren't
				// supported?
			}
		}
		return true;
	 ...
	 *         other action is being taken.
	 *         <p>
	 *         For example, if a file with this name is detected: then a
	 *         subclass might prompt the user with an error dialog and return
	 *         <code>false</code>.
	 */
	...
	/**
	 * Update the enabled state of the next button. This is automatically called
	 * when the current displayed directory changes.
	 */
	...
	/**
	 * Update the enabled state of the back button. This is automatically called
	 * when the current displayed directory changes.
	 */
	...
	/**
	 * Update the enabled state of the up button. This is automatically called
	 * when the current displayed directory changes.
	 */
	...
	/**
	 * Update the enabled state of the new folder button. This is automatically
	 * called when the current displayed directory changes.
	 */
	 ...
	 * <p>
	 * Subclasses must increment the protected field "adjustingComboBox" before
	 * changing the contents/selection of this combobox, otherwise events will
	 * be inappropriately fired. (Then decrement the field again when finished.)
	 * <p>
	 * This is automatically called when the current displayed directory
	 * changes.
	 */
	protected void updateComboBox() {
		...
		try {
			...
			IOLocation current = locationPane.getLocationHistory()
					.getLocation();
			while (current != null) {
	...
	/**
	 * This calls updateNextButton(), updateBackButton(),
	 * updateNewFolderButton(), updateUpButton(), and updateComboBox().
	 * 
	 */
	...
	/**
	 * The default name for a new directory. This may prompt the user for a
	 * dialog.
	 * 
	 * @return if this returns <code>null</code> then no directory should be
	 *         created.
	 */
	protected abstract String getNewFolderName();

	/**
	 * This is called when the user clicks the <code>newFolderButton</code>. The
	 * default implementation uses <code>getNewFolderName()</code>, possibly
	...
	protected void createNewFolder() {
		try {
			...
			IOLocation directory = locationPane.getLocationHistory()
					.getLocation();
	...
	/**
	 * This is used in <code>createNewFolder()</code> to decide whether to open
	 * newly created folders, or merely select them.
	 * 
	...
	/**
	 * This is a convenience method to help create a new folder. This will
	 * guarantee that the directory will be visible, refresh the browser, select
	 * the newly created directory, and any other misc GUI needs.
	 * <p>
	 * This is called by <code>createNewFolder()</code>.
	 * 
	 * @param newDirectory
	...
	protected void createNewFolder(final IOLocation newDirectory) {
		try {
			...
			if (isNewFolderOpened()) {
				...
				locationPane.getLocationHistory().append(
						newDirectory.getParent());
			}

			/**
			 * Suppose the browser takes X ms to refresh: This new folder can't
			 * be selected in the browser until it exists there. So we at least
			 * can add a little delayed response to help. What's a more elegant
			 * solution? worst case scenario: the new directory isn't selected.
