...
import jshdesktop.com.pump.swing.JGradientSlider;
import jshdesktop.com.pump.swing.MultiThumbSlider;

/**
 * The UI for the GradientSlider class.
 * 
 * There are a few properties you can use to customize the UI of a
 * GradientSlider. You can set these for each slider by calling: <BR>
 * <code>slider.putClientProperty(key,value);</code>
 * <P>
 * Or you can set these globally by calling: <BR>
 * <code>UIManager.put(key,value);</code>
 * <P>
 * The available properties are:
 * <P>
 * <TABLE summary="GradientSliderUI Client Properties" BORDER="1" CELLPADDING=5>
 * <TR>
 * <TD>Property Name</TD>
 * <TD>Default Value</TD>
 * <TD>Description</td>
 * </TR>
 * <TR>
 * <TD>GradientSlider.useBevel</TD>
 * <TD>"false"</TD>
 * <TD>If this is <code>true</code>, then this slider will be painted in a
 * rectangle with a bevel effect around the borders. If this is
 * <code>false</code>, then this slider will be painted in a rounded rectangle.</td>
 * </TR>
 * <TR>
 * <TD>GradientSlider.showTranslucency</TD>
 * <TD>"true"</TD>
 * <TD>If this is <code>true</code>, then the slider will reflect the opacity of
 * the colors in the gradient, and paint a checkered background underneath the
 * colors to indicate opacity. If this is <code>false</code>, then this slider
 * will always paint with completely opaque colors, although the actual colors
 * may be translucent.</td>
 * </TR>
 * <TR>
 ...
 * </TR>
 * <TR>
 * <TD>MultiThumbSlider.indicateComponent</TD>
 * <TD>"true"</TD>
 * <TD>If this is <code>true</code>, then the thumbs will only paint on this
 * component when the mouse is inside this slider <i>or</i> when this slider as
 * the keyboard focus.</td>
 * </TR>
 * <TR>
 * <TD>MultiThumbSlider.indicateThumb</TD>
 * <TD>"true"</TD>
 * <TD>If this is <code>true</code>, then the thumb the mouse is over will
 * gently fade into a slightly different color.</td>
 * </TR>
 * </TABLE>
 *
 */
public class GradientSliderUI extends MultiThumbSliderUI<Color> {
	int TRIANGLE_SIZE = 8;
	/**
	...
	protected void calculateImage() {
		...
		boolean alwaysOpaque = getProperty(slider,
				"GradientSlider.showTranslucency", "true").equals("false");
	...
	private static void createCheckerPaint() {
		int k = 4;
		BufferedImage bi = new BufferedImage(2 * k, 2 * k,
		...
		g.fillRect(0, 0, k, k);
		g.fillRect(k, k, k, k);
		checkerPaint = new TexturePaint(bi, new Rectangle(0, 0, bi.getWidth(),
				bi.getHeight()));
	}
	...
	private Shape getFrame() {
		...
		if (getProperty(slider, "GradientSlider.useBevel", "false").equals(
				"true")) {
			return trackRect;
		}
		...
		if (slider.getOrientation() == MultiThumbSlider.HORIZONTAL) {
			...
			return new RoundRectangle2D.Float(trackRect.x - curve, trackRect.y,
					trackRect.width + 2 * curve, trackRect.height, curve * 2,
					curve * 2);
		} else {
			int curve = Math.min(TRIANGLE_SIZE - 2, trackRect.width / 2);
			return new RoundRectangle2D.Float(trackRect.x, trackRect.y - curve,
					trackRect.width, trackRect.height + 2 * curve, curve * 2,
					curve * 2);
		}
	}
	...
	protected void paintTrack(Graphics2D g) {
		...
		TexturePaint tp = new TexturePaint(img, new Rectangle(trackRect.x, 0,
				img.getWidth(), 1));
		g.setPaint(tp);
		...
		if (getProperty(slider, "GradientSlider.useBevel", "false").equals(
			...
			if (slider.getOrientation() == MultiThumbSlider.HORIZONTAL) {
				...
				g.fillRect(trackRect.x + trackRect.width, 0, slider.getWidth()
						- (trackRect.x + trackRect.width), slider.getHeight());
			} else {
				g.clip(frame);
				g.setColor(colors[first]);
