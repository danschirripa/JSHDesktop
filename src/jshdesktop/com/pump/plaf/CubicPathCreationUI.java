...
import jshdesktop.com.pump.geom.CubicPath;
import jshdesktop.com.pump.swing.ShapeCreationPanel;
import jshdesktop.com.pump.swing.ShapeCreationPanel.Active;
import jshdesktop.com.pump.swing.ShapeCreationPanel.DataModel;
import jshdesktop.com.pump.swing.ShapeCreationPanel.DataModelListener;
import jshdesktop.com.pump.swing.ShapeCreationPanel.Handle;
import jshdesktop.com.pump.swing.ShapeCreationPanel.Selection;

/**
 * A {@link ShapeCreationUI} to create a series of {@link CubicPath} shapes.
 * 
 * @see jshdesktop.com.pump.geom.CubicPath
 */
public class CubicPathCreationUI extends ShapeCreationUI {
	...
	public static final String CONSTRAINT_KEY = CubicPathCreationUI.class
			.getName() + ".constraint";
	public static final String SCALE_FACTOR_KEY = CubicPathCreationUI.class
			.getName() + ".scale-factor";

	/** Maps to an array of CubicPaths for editing. */
	protected static final String SHAPES_MIRROR = CubicPathCreationUI.class
	...
	MouseInputAdapter mouseListener = new MouseInputAdapter() {
		...
		public void mousePressed(MouseEvent e) {
			...
			if (isCreating) {
				int selectedShape = scp.getSelectionModel().getSelection()
						.getShapeIndex();
				if (selectedShape == -1) {
					GeneralPath path = new GeneralPath();
					path.moveTo(x, y);
					int newIndex = scp.getDataModel().addShape(path);
					scp.getSelectionModel().select(newIndex, 0,
							Handle.NEXT_CONTROL);
				} else {
					/**
					 * We didn't give a next control point to the *last* node
					 * yet. Now we do. If the last node in a path has a next
					 * control point: that path will be closed.
					 * 
					 */
					...
					Point2D lastPointCtrlPoint = path.getPrevControlForNode(i,
							null);
					if (lastPointCtrlPoint == null)
						lastPointCtrlPoint = lastPoint;
					...
					path.setNextControlForNode(
							i,
							new Point2D.Double(lastPoint.getX() + dx, lastPoint
									.getY() + dy));
		 ...
		 * 
		 * @param mouseLoc
		 *            the mouse location, relative to the panel (pre-transform)
		 */
			...
			for (int shapeIndex = paths.length - 1; shapeIndex >= 0; shapeIndex--) {
				if (active.supports(scp, shapeIndex)) {
					...
					for (int nodeIndex = 0; nodeIndex < path.getNodeCount(); nodeIndex++) {
						Point2D p2 = path
								.getPrevControlForNode(nodeIndex, null);
						if (p2 != null) {
							tx.transform(p2, p2);
							if (hit(scp, mouseLoc, p2)) {
								return new Selection(shapeIndex, nodeIndex,
										Handle.PREVIOUS_CONTROL);
							}
						}
					...
					if (shapeIntersection == null) {
						Shape outline = new BasicStroke(1,
								BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND)
								.createStrokedShape(path);
						if (outline.intersects(mouseLoc.getX() - r,
								mouseLoc.getY() - r, 2 * r, 2 * r)) {
		...
		private boolean hit(ShapeCreationPanel scp, Point2D p1, Point2D p2) {
			...
			size /= 2.0;
			return (Math.abs(p1.getX() - p2.getX()) < size && Math.abs(p2
					.getY() - p1.getY()) < size);
		}
		...
		public void mouseDragged(MouseEvent e) {
			try {
				...
				if (isCreating) {
					...
					float dx = x - clickX;
					float dy = y - clickY;
					path.setPrevControlForNode(nodeIndex, clickX - dx, clickY
							- dy);
				} else {
					...
					if (nodeIndex != -1) {
						if (Handle.PREVIOUS_CONTROL.equals(s.getHandle())) {
							path.setPrevControlForNode(nodeIndex,
									new Point2D.Double(x, y));
							if (Constraint.ANGLE_ONLY
									.equals(getConstraint(scp))) {
								path.setNextControlForNodeFromPrev(nodeIndex,
										false);
							} else if (Constraint.ANGLE_AND_DISTANCE
									.equals(getConstraint(scp))) {
								path.setNextControlForNodeFromPrev(nodeIndex,
										true);
							}
						} else if (Handle.PRIMARY.equals(s.getHandle())) {
							path.setNode(nodeIndex, new Point2D.Double(x, y),
									true);
						} else { // next control:
							path.setNextControlForNode(nodeIndex,
									new Point2D.Double(x, y));
							if (Constraint.ANGLE_ONLY
									.equals(getConstraint(scp))) {
								path.setPrevControlForNodeFromNext(nodeIndex,
										false);
							} else if (Constraint.ANGLE_AND_DISTANCE
									.equals(getConstraint(scp))) {
								path.setPrevControlForNodeFromNext(nodeIndex,
				...
				if (scp.getHandlesActive().supports(scp, shapeIndex)) {
					...
					if (path != null && path.isEmpty() == false) {
						for (int nodeIndex = 0; nodeIndex < path.getNodeCount(); nodeIndex++) {
							...
							Point2D p = path.getPrevControlForNode(nodeIndex,
									null);
							...
							if (p != null) {
								...
								if (selection.getShapeIndex() == shapeIndex
										&& selection.getNodeIndex() == nodeIndex
										&& Handle.NEXT_CONTROL.equals(selection
												.getHandle())) {
									g.setColor(Color.black);
								} else if (indication.getShapeIndex() == shapeIndex
										&& indication.getNodeIndex() == nodeIndex
							...
							r.setFrame(nodePoint.getX() - z, nodePoint.getY()
									- z, 2 * z, 2 * z);

							if (selection.getShapeIndex() == shapeIndex
									&& selection.getNodeIndex() == nodeIndex
									&& Handle.PRIMARY.equals(selection
											.getHandle())) {
								g.setColor(Color.black);
							} else if (indication.getShapeIndex() == shapeIndex
									&& indication.getNodeIndex() == nodeIndex
									&& Handle.PRIMARY.equals(indication
											.getHandle())) {
								g.setColor(Color.gray);
							} else {
								g.setColor(Color.white);
