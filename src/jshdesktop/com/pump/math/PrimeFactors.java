...
import jshdesktop.com.pump.math.MathException.NegativeException;
...
public class PrimeFactors {
	...
	public static long[] getPrimeNumbers(int arrayLength) {
		...
		while (primes.size() < arrayLength) {
			try {
				long[] factors = get(a);
				for (int z = 0; z < factors.length
						&& primes.size() < arrayLength; z++) {
	...
	/**
	 * Return the prime factors of a number. This caches data to a static
	 * look-up table.
	 * 
	 * @param i
