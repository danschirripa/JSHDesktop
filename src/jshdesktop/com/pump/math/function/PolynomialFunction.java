 ...
 */
package jshdesktop.com.pump.math.function;

import java.util.ArrayList;
import java.util.List;

import jshdesktop.com.pump.math.Equations;
...
public class PolynomialFunction implements Function {
	...
	/**
	 * Creates a linear <code>PolynomialFunction</code> that passes through the
	 * two points provided.
	 * 
	 * @param x1
	 ...
	 * @return a <code>PolynomialFunction</code> that passes through the points
	 *         provided.
	 */
	public static PolynomialFunction createFit(double x1, double y1, double x2,
			double y2) {
		return new PolynomialFunction(new double[] { (y2 - y1) / (-x1 + x2),
				(y1 * x2 - y2 * x1) / (-x1 + x2) });
	}
	 ...
	 * @param ys
	 *            an array of y-coordinates. Each element in this array
	...
	public static PolynomialFunction createFit(double[] xs, double[] ys) {
		if (ys.length != xs.length)
			throw new IllegalArgumentException("xs.length (" + xs.length
					+ ") != ys.length (" + ys.length + ")");
	 ...
	 * Creates a <code>PolynomialFunction</code> that uses the coordinates
	 * provided. <br>
	 * The function returned will pass through all the points provided, with the
	 * dy/dx values provided.
	 * 
	 * @param xs
	 *            an array of x-coordinates.
	 * @param ys
	 ...
	 *            corresponds to an element of the x coordinates.
	 * @param yDerivatives
	 *            an array of dy/dx values. Each element in this array
	 *            corresponds to an element of the x coordinates.
	 * @return a function that matches the coordinates provided.
	 */
	public static PolynomialFunction createFit(double[] xs, double[] ys,
		...
		if (ys.length != yDerivatives.length)
			throw new IllegalArgumentException("ys.length (" + ys.length
					+ ") != yDerivatives.length (" + yDerivatives.length + ")");
		if (ys.length != xs.length)
			throw new IllegalArgumentException("xs.length (" + xs.length
					+ ") != ys.length (" + ys.length + ")");
		...
		for (int row = 0; row < coefficientsMatrix.length; row += 2) {
			...
			for (int column = 0; column < coefficientsMatrix[row].length - 1; column++) {
				...
				if (power == 0) { // no derivative for this one
					...
					coefficientsMatrix[row + 1][column] = power
							* Math.pow(xs[row / 2], power - 1);
				}
			}
