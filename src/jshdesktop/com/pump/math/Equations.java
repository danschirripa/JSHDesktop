 ...
 */
package jshdesktop.com.pump.math;
...
public class Equations {
	 ...
	 * <P>
	 * For example, if you have the three equations: <BR>
	 * 2x + y + z = 1 <BR>
	 * 6x + 2y + z = -1 <BR>
	 * -2x + 2y + z = 7
	 * <P>
	 * You can call: <code><BR>double[][] coeffs = { { 2, 1, 1, 1},</code>
	 ...
	 * <code><BR>{ { 1, 0, 0, -1}, </code> <code><BR>  { 0, 1, 0, 2}, </code>
	 ...
	 * <P>
	 * Note that due to the limits of double-precision, some safeguards were
	 * built-in that may round values less than .00000001 to zero. If you need
	 * better precision, you may have to copy and paste the algorithm in this
	 * code and use a more precise number format.
	 * 
	 * @param coefficients
	 *            the matrix of coefficients. It must be N x (N+1) units in
	 *            size.
	 * @throws IllegalArgumentException
	 *             if it is impossible to solve for each variable. In this case
	 *             an exception is thrown, and no guarantee is made regarding
	 *             the final state of the <code>coefficients</code> matrix.
	 *             <P>
	 *             Also an exception may be thrown if the matrix is not
	 *             correctly sized.
	 */
	 ...
	 *            size.
	 * @param sort
	 *            If this is <code>true</code>, then you're guaranteed for your
	 *            final solution to be sorted such that the 1's form a diagonal
	 *            line from [0,0] to [N,N]. If this is <code>false</code>, then
	 *            the solution <i>may</i> not form a diagonal line of 1's.
	 *            <P>
	 *            (If there are no zeroes in the matrix, the diagonal line will
	 *            be there by default, but if zeroes are present then some rows
	 *            may have to be skipped and the normal order is disrupted.)
	 *            <P>
	 *            The sort is performed by calling <code>Arrays.sort()</code>.
	 * @throws IllegalArgumentException
	 *             if it is impossible to solve for each variable. In this case
	 *             an exception is thrown, and no guarantee is made regarding
	 *             the final state of the <code>coefficients</code> matrix.
	...
	public static void solve(double[][] coefficients, boolean sort) {
		...
		// but if a coefficient somewhere is already zero, then we may have to
		// skip around.
		...
		while (ctr < b.length) {
			if (coefficients[row].length != size + 1)
				throw new IllegalArgumentException(
						"The matrix must be N x (N+1) units long.  The matrix provided is "
								+ size + " x " + coefficients[row].length
								+ " units.");
			if (b[row] == false
					&& Math.abs(coefficients[row][ctr]) > .0000000001) {

				errorCounter = 0;
				// we haven't considered this row yet

				// first we make the value at [row][ctr] a 1.0, by multiplying
				// the entire row by a constant:
				t = 1 / coefficients[row][ctr];
				for (a = 0; a < coefficients[row].length; a++) {
					coefficients[row][a] *= t;
				}
				coefficients[row][ctr] = 1.0; // to make sure, despite possible
												// arithmetic rounding, that we
												// get 1.0
			...
			if (errorCounter > coefficients.length) {
				if (VERBOSE_EXCEPTIONS) {
					throw new IllegalArgumentException(
							"The coefficient matrix cannot be solved.  Either it has infinitely many solutions, or zero solutions:\n"
		...
		for (a = order.length - 2; a >= 0; a--) {
			...
			for (i = coefficients[row].length - 2; i > a; i--) {
				t = coefficients[row][i]
						* coefficients[order[i]][coefficients[row].length - 1];
	...
	public static void solve(BigDecimal[][] coefficients, boolean sort) {
			...
			if (coefficients[row].length != size + 1)
				throw new IllegalArgumentException(
						"The matrix must be N x (N+1) units long.  The matrix provided is "
								...
								+ " units.");
			if (b[row] == false
					&& coefficients[row][ctr].compareTo(tolerance) > 0) {
				...
				coefficients[row][ctr] = BigDecimal.ONE; // to make sure,
															// despite possible
															// arithmetic
															// rounding, that we
															// get 1.0
					...
					if (b[a] == false) {
						...
						for (i = 0; i < coefficients[a].length; i++) {
							coefficients[a][i] = coefficients[a][i]
									.subtract(coefficients[row][i].multiply(t));
						}
					}
				}
				// we've now considered this row
		...
		for (a = order.length - 2; a >= 0; a--) {
			...
			for (i = coefficients[row].length - 2; i > a; i--) {
				t = coefficients[row][i]
						.multiply(coefficients[order[i]][coefficients[row].length - 1]);
