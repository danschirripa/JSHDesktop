...
public class AWTMonitorDemo extends ShowcaseDemo {
	...
	public AWTMonitorDemo() {
		...
		lockButton.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent e) {
				try (PrintStream ps = console.createPrintStream(false)) {
					...
					ps.println("--------------------------------");
					ps.println(
							"I'm about to block the event dispatch thread for 5 seconds.");
					ps.println(
							"You won't be able to interact with this application during this time...");
					ps.println();
				}
					...
					public void actionPerformed(ActionEvent e) {
						...
						try {
							...
							try (PrintStream ps = console
								...
								ps.println(
										"The thread that was blocking exited normally. (It wasn't aborted.)");
							}
						} catch (ThreadDeath td) {
							try (PrintStream ps = console
									.createPrintStream(false)) {
								ps.println(
										"The thread that was blocking was forcibly stopped.");
							}
						} finally {
		...
		stopThreadCheckBox.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (stopThread)
					QDialog.showDialog(frame, "Warning",
	...
	private void initialize() {
		...
		AWTMonitor
				.createAWTMonitorThread("Showcase App", 2000, 4000,
						"AWT Monitor Demo", console.createPrintStream(false))
				.start();
		...
		AWTMonitor.addPanicListener(new AWTPanicListener() {
			...
			public void AWTPanic(String applicationName) {
				if (blockingThread != null && stopThread) {
					try (PrintStream ps = console.createPrintStream(false)) {
						ps.println(
								"Calling Thread.stop on the event dispatch thread...");
						ps.println(
								"(If you see this message, then everything probably worked!");
					}
					blockingThread.stop();
					blockingThread = null;
					try {
