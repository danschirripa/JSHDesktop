 ...
 */
package jshdesktop.com.pump.debug;
...
/**
 * This controversial class launches a new process to ask the user if they want
 * to try to abort the event dispatch thread (by calling Thread.stop()).
 * <P>
 * There are several legitimate reasons to disparage this class:
 * <ul>
 * <li>Thread.stop() has been deprecated for ages, and is unsafe.</li>
 * <li>Launching another process is a tacky solution.</li>
 * <li>The dialog that results is hard for a user to understand.</li>
 * </ul>
 * <P>
 * ... and the list goes on. That being said: this class has saved my program
...
public class PanicDialogPrompt implements AWTPanicListener {
	...
	/**
	 * This launches another process to prompt the user to cancel the current
	 * activity. If the other process indicates that the user wants to cancel,
	 * then this method will call <code>stopAWTThread</code>
	 * 
	 * @param applicationName
	 *            the name to present to the user of the application this might
	 *            affect.
	 */
	...
	/**
	 * This uses the unsafe <code>Thread.stop()</code> method on the AWT thread
	 * to try to regain control of the application.
	 */
		...
		for (int a = 0; a < array.length; a++) {
			...
			if (array[a].getName().indexOf("AWT-EventQueue-") == 0) {
				try {
					...
					System.err
							.println("Attempted unsuccessfully to stop the AWT thread.");
				}
			}
		}
	}
