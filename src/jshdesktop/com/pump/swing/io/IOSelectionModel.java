...
/**
 * A selection model for <code>LocationPanes</code> and
 * <code>LocationBrowsers</code>.
 */
				...
				for (int a = 1; a < newSelection.length; a++) {
					if (newSelection[a].getParentPath() != null)
						throw new IllegalArgumentException(
								"all elements in the selection must have the same parent");
				}
			} else {
				for (int a = 1; a < newSelection.length; a++) {
					if (newSelection[a].getParentPath().equals(parent) == false) {
						System.out.println(parent + " != "
								+ newSelection[a].getParentPath());
						throw new IllegalArgumentException(
								"all elements in the selection must have the same parent");
					}
				}
