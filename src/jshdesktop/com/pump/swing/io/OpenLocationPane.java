...
import jshdesktop.com.pump.plaf.OpenLocationPaneUI;
...
public class OpenLocationPane extends LocationPane {
	...
	public static IOLocation[] showDialog(Frame parent, String[] suffixes,
			boolean multipleSelection, String title) {
		OpenLocationPane openPane = new OpenLocationPane(suffixes,
				multipleSelection);
	...
	public IOLocation[] showDialog(Frame parent, String title) {
		...
		d.setVisible(true);
		if (returnValue[0] == null) { // remember this is an array, not an
			// object
	...
	public OpenLocationPane(String[] suffixes, boolean multipleSelection) {
		this(new LocationHistory(), new IOSelectionModel(multipleSelection),
				new GraphicCache());
	...
	public OpenLocationPane(LocationHistory directoryStack,
	...
	public void updateUI() {
		if (UIManager.getDefaults().get(uiClassID) == null) {
			UIManager.getDefaults().put(uiClassID,
					"com.pump.plaf.AquaOpenLocationPaneUI");
			// UIManager.getDefaults().put(uiClassID,
			// "com.pump.plaf.XPOpenLocationPaneUI");
		}
		try {
			String className = UIManager.getDefaults().getString(uiClassID);
			Class<?> classObject = Class.forName(className);
			Constructor<?> constructor = classObject
					.getConstructor(new Class[] { LocationPane.class });
			setUI((OpenLocationPaneUI) constructor
