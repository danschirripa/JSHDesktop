...
import jshdesktop.com.pump.io.HTMLEncoding;

/**
 * This helps serialize and deserialize simple UIs based on the names of
 * components. For example, if you have a selected JCheckBox with the name
 * "is-preloaded", then after calling {@link #catalog(Container)} on any parent
 * of that JCheckBox this ContainerProperties will include "is-preloaded=true".
 * Similarly when you call {@link #install(Container, UncaughtExceptionHandler)}
 * it will parse that value and set the UI accordingly.
 * <p>
 * This only supports a small set of data types in JToggleButtons,
 * JTextComponents, JComboBoxes and JLists: primitives, Strings, BigDecimals,
 * BigIntegers, and Files. It's possible this object can serialize values but it
 * won't be able to deserialize them later.
 * <p>
 * This is a quick and light tool intended to help with settings, but it is not
...
public class ContainerProperties extends Properties {
	 ...
	 */
	public static final String KEY_PROPERTY_NAME = ContainerProperties.class
			+ "#property-name";
	...
	/**
	 * Create a ContainerProperties that catalogs all descendants of the
	 * argument.
	 * 
	 * @param container
	 *            a Container with textfields, buttons, comboboxes, etc. that
	 *            you want to catalog with this ContainerProperties.
	 */
		...
		scanChildren: for (int a = 0; a < container.getComponentCount(); a++) {
			...
			if (name != null) {
				if (comp instanceof JTextComponent) {
					...
					System.err.println("unsupported component: "
	...
	private static UncaughtExceptionHandler DEFAULT_HANDLER = new UncaughtExceptionHandler() {
		...
		public void uncaughtException(Thread t, Throwable e) {
			...
			System.err
					.println("An error occurred installing this ContainerProperties:");
	 ...
	 * 
	 * @param container
	 *            Configure all the components in the argument that match
	 *            properties stored in this ContainerProperties.
	 * @param handler
	 *            this will be notified as exceptions come up. If this is null,
	 *            then a default exception handler is used that quietly prints
	 *            messages to System.err but continues to iterate over all
	 *            possible components.
	 */
			...
			if (name != null) {
				...
				if (name != null && value != null) {
					if (comp instanceof JTextComponent) {
						...
						((JToggleButton) comp).setSelected(Boolean
								.parseBoolean(value));
						continue scanChildren;
					} else if (comp instanceof JComboBox<?>) {
						JComboBox<?> jcb = (JComboBox<?>) comp;
								...
								if (j == -1) {
									handler.uncaughtException(
											...
											new IllegalStateException(
													"the list \""
															+ name
															+ "\" doesn't appear to contain the value \""
															+ v + "\""));
								} else {
	...
	protected String getName(Component comp) {
		...
		if (comp instanceof JComponent)
			name = (String) ((JComponent) comp)
					.getClientProperty(KEY_PROPERTY_NAME);
		if (name == null)
			name = comp.getName();
		return name;
	}
