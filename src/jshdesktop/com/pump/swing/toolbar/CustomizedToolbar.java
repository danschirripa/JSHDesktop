 ...
 */
package jshdesktop.com.pump.swing.toolbar;
...
import jshdesktop.com.pump.awt.AnimatedLayout;
import jshdesktop.com.pump.swing.MockComponent;

/**
 * This toolbar mimick's Apple's standard toolbars and their drag-and-drop
 * customization.
 * <P>
 * There are certain assumptions made about this component, such as:
 * <ul>
 * <LI>This will be given lots of horizontal space.</li>
 * <LI>Components you try to display in it will be of similar "reasonable"
 * heights.</li>
 * <LI>This will be used in a JFrame.</li>
 * <LI>You will not need to call <code>setVisible()</code> on components in this
 * toolbar.</LI>
 * </ul>
 * <P>
 * There is room for improvement in this implementation. As of this writing:
 * <ul>
 * <LI>The "show icon/text" combobox is not yet implemented.</li>
 ...
 * <LI>Also there are at least 2 TODO notes in this class itself, commenting on
 * aesthetic bugs.</LI>
 * </ul>
 * 
 * @see <a
...
public class CustomizedToolbar extends JPanel {
	...
	static final Preferences prefs = Preferences
			.userNodeForPackage(CustomizedToolbar.class);
	...
	/**
	 * Whether a drag is originating from the toolbar, or the dialog. This can
	 * make a big difference on how the drag is treated.
	 */
	boolean draggingFromToolbar;

	/**
	 * Whether the default set of controls is being dragged to the toolbar.
	 */
	boolean draggingDefaults;

	/**
	 * The minimum height of this toolbar, based on the preferred height of all
	 * its components (whether they are visible or not.
	 */
	int minimumHeight;
	 ...
	 */
	protected static boolean hideActiveComponents = DragSource
	...
	/**
	 * Returns a clone of this array, using the same component type the argument
	 * uses.
	 * 
	 * @param array
	 *            the array to clone
	...
	private static <T> T[] insert(T[] array, Object newObject, int index) {
		...
		System.arraycopy(array, index, newArray, index + 1, array.length
				- index);
		return (T[]) newArray;
	}
	...
	public CustomizedToolbar(JComponent[] components, String[] defaults,
		...
		if (isEmpty()) {
			...
			for (int a = 0; a < defaults.length; a++) {
				if (defaults[a] == null)
					throw new NullPointerException("defaults[" + a
							+ "] is null");
				prefs.put(base + a, defaults[a]);
			}
		}
		setMinimumSize(new Dimension(5, minimumHeight + componentInsets.top
				+ componentInsets.bottom));
		setPreferredSize(new Dimension(5, minimumHeight + componentInsets.top
				+ componentInsets.bottom));
		setMaximumSize(new Dimension(5, minimumHeight + componentInsets.top
				+ componentInsets.bottom));
	...
	/**
	 * Returns the components that may or may not be visible in this toolbar,
	 * depending on how the user has configured this toolbar.
	 */
	...
	/**
	 * Stores the component names (in order of appearance) that this toolbar
	 * should display in the preferences.
	 */
	...
	public void displayDialog(int dialogMaxWidth) {
		...
		if (parent == null)
			throw new NullPointerException(
					"the dialog cannot be shown if the parent frame is null.");

		putClientProperty(DIALOG_ACTIVE, Boolean.TRUE); // makes spaces paint
														// themselves
		CustomizedToolbarOptions options = new CustomizedToolbarOptions(this,
				dialogMaxWidth);

		final JComponent modalCover = new JComponent() {
			private static final long serialVersionUID = 1L;
		};
		dragSource.createDefaultDragGestureRecognizer(modalCover,
				DnDConstants.ACTION_COPY_OR_MOVE, dragGestureListener);
		new DropTarget(modalCover, dropTargetListener);

		/**
		 * Having a MouseListener -- even if its empty -- prevents MouseEvents
		 * from leaking through to components underneath.
		 */
		...
		FakeSheetWindowListener windowListener = new FakeSheetWindowListener(
				parent, dialog, this, modalCover);
