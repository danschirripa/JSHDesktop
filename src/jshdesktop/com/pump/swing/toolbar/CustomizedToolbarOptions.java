...
class CustomizedToolbarOptions extends JPanel {
	...
	/**
	 * Analogous to the toolbar's list of components, except a MockComponent is
	...
	public CustomizedToolbarOptions(CustomizedToolbar t, int maxWidth) {
		...
		for (int a = 0; a < componentList.length; a++) {
			dragSource.createDefaultDragGestureRecognizer(componentList[a],
		...
		add(mockDefaults, c);
		dragSource.createDefaultDragGestureRecognizer(mockDefaults,
				DnDConstants.ACTION_MOVE, dragGestureListener);
	...
	JLabel label1 = new JLabel(
			"Drag your favorite items into the toolbar\u2026"); // \u2026 is
																// unicode for
																// an ellipsis
	...
	class Shadow extends JComponent {
		...
		protected void paintComponent(Graphics g) {
			...
			boolean dark = CustomizedToolbar.isDarkBackground(SwingUtilities
					.getWindowAncestor(this));
			for (int a = 0; a < getHeight(); a++) {
				float alpha;
				if (dark) {
					alpha = ((float) (getHeight() - a - 1))
							/ ((float) getHeight()) * .35f;
				} else {
					alpha = ((float) (getHeight() - a - 1))
							/ ((float) getHeight()) * .15f;
