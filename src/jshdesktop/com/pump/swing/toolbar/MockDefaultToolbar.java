...
/**
 * This is a rendering of the default toolbar. This may look different from the
 * actual toolbar because all flexible spaces are compacted to the
 * minimum/default space width.
 */
class MockDefaultToolbar extends MockComponent {
	...
	private static BufferedImage createImage(CustomizedToolbar toolbar) {
		...
		BufferedImage bi = new BufferedImage(minWidth, toolbar.minimumHeight
				+ insets.top + insets.bottom, BufferedImage.TYPE_INT_ARGB);
		...
		for (int a = 0; a < contents.length; a++) {
			...
			if (contents[a].length() > 0 && contents[a].charAt(0) == '\t') {
				SpaceComponent.paintSpace(g, false, true, sizes[a].width,
						sizes[a].height);
			} else if (contents[a].length() > 0 && contents[a].charAt(0) == '-') {
				MacToolbarSeparatorUI.paintSeparator(g, sizes[a].width,
						sizes[a].height);
			} else {
