...
public abstract class AutocompleteTextField<T> extends JTextField {
	...
	public static interface AutocompleteModelChangeListener<T> {
		public void selectionChange(T[] suggestions, int oldSelection,
				int oldIndication, int newSelection, int newIndication);

		public void indicationChange(T[] suggestions, int oldIndication,
				int newIndication);

		public void suggestionChange(T[] oldSuggestions, int oldSelection,
				int oldIndication, T[] newSuggestions, int newSelection,
				int newIndication);
	}
	...
	public static class DefaultSuggestionModel<K> implements SuggestionModel<K> {
		...
		public void refreshSuggestions(AutocompleteModel<K> dataModel,
			...
			K[] newArray = (K[]) Array.newInstance(array.getClass()
					.getComponentType(), matchingList.size());
			matchingList.toArray(newArray);
			dataModel.setSuggestions(newArray, newSelectionIndex,
					newIndicationIndex);
		}
	}
	...
	public static interface SuggestionModel<K> {
		 ...
		 * after the contents of this text field change. This is executed on the
		 * EDT, so it needs to be relatively fast (or pass harder work off to
		 * another thread).
		 */
		public void refreshSuggestions(AutocompleteModel<K> dataModel,
				AutocompleteTextField<K> field);
	...
	public static class AutocompleteModel<T> {
		...
		private int cycleSuggestion(int origValue, int delta) {
			if (delta == 0)
				throw new IllegalArgumentException("delta must be nonzero");
			...
			int newIndex = (origValue + delta + suggestions.length)
					% suggestions.length;
			return newIndex;
		...
		public synchronized void setSuggestions(T[] newSuggestions,
				int newSelectedIndex, int newIndicatedIndex) {
			...
			if (listChange) {
				for (AutocompleteModelChangeListener<T> l : changeListeners) {
					try {
						l.suggestionChange(oldSuggestions, oldSelection,
								oldIndication, newSuggestions, selectedIndex,
								indicatedIndex);
				...
				for (AutocompleteModelChangeListener<T> l : changeListeners) {
					try {
						l.selectionChange(newSuggestions, oldSelection,
								oldIndication, selectedIndex, indicatedIndex);
					} catch (RuntimeException e) {
						e.printStackTrace();
					}
				}
				...
				for (AutocompleteModelChangeListener<T> l : changeListeners) {
					try {
						l.indicationChange(newSuggestions, oldIndication,
								indicatedIndex);
					} catch (RuntimeException e) {
						e.printStackTrace();
					}
		...
		private void updatePopupVisibility(T[] newSuggestions, int newIndication) {
			if (newSuggestions != null && newSuggestions.length >= 1) {
				...
				try {
					...
					if (newIndication >= 0) {
						Rectangle r = suggestionList.getCellBounds(
								newIndication, newIndication);
						if (!suggestionList.getVisibleRect().contains(
								r.x + r.width / 2, r.y + r.height / 2)) {
							JScrollBar vbar = scrollPane.getVerticalScrollBar();
							int v = suggestionList.getFixedCellHeight()
									* newIndication;
							vbar.setValue(v);
						}
						// why didn't this work:
						// Rectangle r =
						// suggestionList.getCellBounds(newIndication,
						// newIndication);
	...
	ListSelectionListener listListener = new ListSelectionListener() {
		...
		public void valueChanged(ListSelectionEvent e) {
			getModel().setSuggestions(model.getSuggestions(),
					model.selectedIndex, suggestionList.getSelectedIndex());
		}
	};

	protected SuggestionModel<T> suggestionModel = null;
	protected JList<T> suggestionList = new JList<T>();
	protected JScrollPane scrollPane = new JScrollPane(suggestionList,
	...
	/**
	 * This optional key name combined with the "preference" field is used to
	 * store/retrieve the last value the user applied to this field
	 * 
	 * @see #preferences
	 */
	 ...
	 *         <p>
	 *         This may return false if the only suggestion is already selected,
	...
	protected void showPopup() {
		...
		Dimension d = new Dimension(getWidth(), rowCount
