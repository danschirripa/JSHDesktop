 ...
 * <code>JProgressBar</code> is either not useful (because a task is
 * indeterminate) or too large (because a task must be expressed in a tiny
 * amount of space). Here are some possible UI implementations:
 * <p>
 ...
 * >Apple's OSX Human Interface Guidelines</a> call this an "Asynchronous
 * Progress Indicator", and describe it as follows: <blockquote>An asynchronous
 * progress indicator provides feedback on an ongoing process.
 * <p>
 ...
 * 
 * @see <a href=
 *      "https://javagraphics.blogspot.com/2014/03/implementing-jthrobber.html">Implementing
 *      a JThrobber</a>
 */
public class JThrobber extends JComponent implements Accessible {
	...
	public void updateUI() {
		if (UIManager.getDefaults().get(uiClassID) == null) {
			UIManager.getDefaults().put(uiClassID, "jshdesktop.com.pump.plaf.ThrobberUI");
		}
		setUI(new PulsingCirclesThrobberUI());
	}
	...
	/**
	 * An inactive throbber doesn't paint itself. This is sometimes useful (instead
	 * of simply toggling the visibility of this throbber on/off) because it
	 * maintains the same size in the UI, so other elements won't constantly shift
	 * around but the widget can still effectively be hidden.
	 */
