...
import jshdesktop.com.pump.plaf.AbstractSearchHighlight;

/**
 * Some static methods relating to searching a <code>JTextComponent</code> or
 * <code>JTable</code>.
 * 
 */
public class SwingSearch {
	...
	/**
	 * This performs a basic search, starting from the current selection. Also
	 * this consults <code>UIManager.get("beepWhenSearchFails")</code> to
	 * determine whether to beep or not if a match is not found.
	 * 
	 * @param textComponent
	 ...
	 * @param forward
	 *            whether to move forward or backward (aka "next" or
	 *            "previous").
	 * @param matchCase
	 *            whether to match the case or not.
	 * @return true if a match is found, false otherwise.
	 */
	public static boolean find(JTextComponent textComponent,
			String searchPhrase, boolean forward, boolean matchCase) {
		boolean returnValue = find2(textComponent, searchPhrase, forward,
				matchCase);
		if (!returnValue) {
			notifyFailedSearch();
		}
	 ...
	 * @param searchPhrase
	 *            the phrase we're looking for.
	 * @param matchCase
	 *            whether we have to be case sensitive or not.
	 * @return the number of occurrences of the phrase in the text component.
	 */
	public static int countOccurrence(JTextComponent textComponent,
			String searchPhrase, boolean matchCase) {
		if (textComponent == null)
			throw new NullPointerException(
					"No text component was provided to search.");
		if (searchPhrase == null)
		...
		while (index < docLength) {
			if (find(textComponent, searchPhrase, true, matchCase, false,
					index, docLength, selection)) {
	...
	/**
	 * This performs a basic search, starting from a specific index. If a match
	 * is found, the indices are stored in the <code>selection</code> array, and
	 ...
	 *            whether to match the case or not.
	 * @param startingIndex
	 *            the index in the document to begin searching.
	 * @param selection
	 *            the array to store the indices in if a match is found.
	 * @return true if the array provided now contains two indices enclosing the
	 *         search phrase.
	 */
	public static boolean find(JTextComponent textComponent,
			String searchPhrase, boolean forward, boolean matchCase,
			int startingIndex, int[] selection) {
		...
		if (forward) {
			...
			return find(textComponent, searchPhrase, forward, matchCase, false,
					startingIndex, length, selection);
		} else {
			return find(textComponent, searchPhrase, forward, matchCase, false,
					startingIndex, 0, selection);
		}
	}
	 ...
	 * @param wrapAround
	 * @param startingIndex
	 *            the index at which we start reading
	 * @param endIndex
	 *            the index at which we stop reading
	 * @param selection
	 *            the selection start and end will be stored in this 2-int array
	 * @return true if the "selection" argument is now populated with the
	 *         location of this search phrase
	 */
	private static boolean find(JTextComponent textComponent,
			String searchPhrase, boolean forward, boolean matchCase,
			boolean wrapAround, int startingIndex, int endIndex, int[] selection) {
		/**
		 ...
		 * a String, and then use indexOf and lastIndexOf to perform these
		 * searches: but it seems like that might be really wasteful in terms of
		 * large documents. (Especially consider the countOccurrence method,
		 * which may call this method thousands of times with every keystroke.)
		 * 
		 * So -- while I admit I don't know what I'm doing -- it seems like
		 * using the Segment class might be the better way to do this?
		 */
		...
		try {
			if (forward) { // searching forward
				...
				while (index < endIndex) {
					...
					int searchLength = remainingCharsInDoc > remainingCharsInSearchPhrase ? remainingCharsInSearchPhrase
							: remainingCharsInDoc;
					...
					if (matchCase == false) {
						for (int a = 0; a < segment.count; a++) {
							array[a] = Character.toUpperCase(segment.array[a
									+ segment.offset]);
						}
						workingArray = array;
						arrayOffset = 0;
					...
					scan: {
						for (int a = 0; a < segment.count; a++) {
							if (searchPhrase.charAt(matching) == workingArray[arrayOffset
									+ a]) {
								matching++;
								if (matching == spl) {
									selection[1] = index + matching
											- (spl - segment.count);
				...
				if (wrapAround && startingIndex != 0) {
					startingIndex = Math.min(textComponent.getDocument()
							.getLength(), startingIndex + spl);
					return find(textComponent, searchPhrase, forward,
							matchCase, false, 0, startingIndex, selection);
				}
				return false;
				...
				while (index > endIndex) {
					...
					scan: {
						for (int a = segment.count - 1; a >= 0; a--) {
							if (searchPhrase.charAt(spl - 1 - matching) == workingArray[arrayOffset
									+ a]) {
								matching++;
								if (matching == spl) {
									selection[0] = index + matching
											- (segment.count - 1 - a) - 1;
				...
				if (wrapAround && startingIndex != docLength) {
					...
					return find(textComponent, searchPhrase, forward,
							matchCase, false, docLength, startingIndex,
							selection);
				}
				return false;
			}
	 ...
	 * {@link jshdesktop.com.pump.plaf.AbstractSearchHighlight} class, but it can work with
	 ...
	 *            the beginning of the selection to highlight.
	 * @param selectionEnd
	 *            the end of the selection to highlight.
	 */
	public static void highlight(JTextComponent tc, int selectionStart,
		...
		try {
			...
			Constructor<?> constructor = c.getConstructor(new Class[] {
					JTextComponent.class, Integer.TYPE, Integer.TYPE });
			constructor.newInstance(new Object[] { tc,
					new Integer(selectionStart), new Integer(selectionEnd) });
		} catch (Throwable t) {
