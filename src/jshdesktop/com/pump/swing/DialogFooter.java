 ...
 */
package jshdesktop.com.pump.swing;
...
import jshdesktop.com.pump.plaf.FocusArrowListener;
import jshdesktop.com.pump.util.JVM;

/**
 * This is a row of buttons, intended to be displayed at the bottom of a dialog.
 * This class is strongly related to the {@link jshdesktop.com.pump.swing.QDialog} project,
 * although the <code>DialogFooter</code> can exist by itself.
 * <P>
 * On the left of a footer are controls that should apply to the dialog itself,
 * such as "Help" button, or a "Reset Preferences" button. On the far right are
 * buttons that should dismiss this dialog. They may be presented in different
 * orders on different platforms based on the <code>reverseButtonOrder</code>
 * boolean.
 * <P>
 * Buttons are also generally normalized, so the widths of buttons are equal.
 * <P>
 * This object will "latch onto" the RootPane that contains it. It is assumed
 * two DialogFooters will not be contained in the same RootPane. It is also
 * assumed the same DialogFooter will not be passed around to several different
 * RootPanes.
 * <h3>Preset Options</h3> This class has several OPTION constants to create
 * specific buttons.
 * <P>
 * In each constant the first option is the default button unless you specify
 * otherwise. The Apple Interface Guidelines advises: "The default button should
 * be the button that represents the action that the user is most likely to
 * perform if that action isn't potentially dangerous."
 * <P>
 ...
 * the user performs when pressing the button-for example, Erase, Save, or
 * Delete." So instead of presenting a YES_NO dialog with the question
 * "Do you want to continue?" a better dialog might provide the options "Cancel"
 * and "Continue". In short: we as developers might tend to lazily use this
 * option and phrase dialogs in such a way that yes/no options make sense, but
 * in fact the commit buttons should be more descriptive.
 * <P>
 * Partly because of the need to avoid yes/no questions,
 * <code>DialogFooter</code> introduces the dialog type:
 * SAVE_DONT_SAVE_CANCEL_OPTION. This is mostly straightforward, but there is
 * one catch: on Mac the buttons are reordered: "Save", "Cancel" and
 * "Don't Save". This is to conform with standard Mac behavior. (Or, more
 * specifically: because the Apple guidelines state that a button that can cause
 * permanent data loss be as physically far from a "safe" button as possible.)
 * On all other platforms the buttons are listed in the order "Save",
 * "Don't Save" and "Cancel".
 * <P>
 * Also note the field {@link #reverseButtonOrder} controls the order each
 * option is presented in the dialog from left-to-right.
 * <h3>Platform Differences</h3>
 * These are based mostly on studying Apple and Vista interface guidelines.
 * <ul>
 * <LI>On Mac, command-period acts like the escape key in dialogs.</li>
 * <LI>On Mac the Help component is the standard Mac help icon. On other
 * platforms the help component is a {@link jshdesktop.com.pump.swing.JLink}.</li>
 * <LI>By default button order is reversed on Macs compared to other platforms.
 * See the <code>DialogFooter.reverseButtonOrder</code> field for details.</li>
 * <LI>There is a static boolean to control whether button mnemonics should be
 * universally activated. This was added because when studying Windows XP there
 * seemed to be no fixed rules for whether to use mnemonics or not. (Some
 * dialogs show them, some dialogs don't.) So I leave it to your discretion to
 * activate them. I think this boolean should never be activated on Vista or
 * Mac, but on XP and Linux flavors: that's up to you. (Remember using the alt
 * key usually activates the mnemonics in most Java look-and-feels, so just
 * because they aren't universally active doesn't mean you're hurting
...
public class DialogFooter extends JPanel {
	...
	public static enum EscapeKeyBehavior {
		/**
		 * This (the default behavior) does nothing when the escape key is
		 * pressed.
		 */
		DOES_NOTHING,
		 ...
		 * cancel button is present: this does nothing. (Also on Macs
		 * command+period acts the same as the escape key.)
		 * <p>
		 * This should only be used if the cancel button does not lead to data
		...
		/**
		 * This triggers the default button when the escape key is pressed. If
		 * no default button is defined: this does nothing. (Also on Macs
		 * command+period acts the same as the escape key.)
		 * <p>
		 * This should only be used if the default button does not lead to data
		 * loss, because users may quickly press the escape key before reading
		 * the text in a dialog.
		 */
		TRIGGERS_DEFAULT,
		 ...
		 * If no non-default button is defined: this does nothing. (Also on Macs
		 * command+period acts the same as the escape key.)
		 * <p>
		 * This should only be used if the non-default button does not lead to
		 * data loss, because users may quickly press the escape key before
		 * reading the text in a dialog.
		 */
		TRIGGERS_NONDEFAULT
	}
	...
	public static ResourceBundle strings = ResourceBundle
			.getBundle("com.pump.swing.DialogFooter");

	/**
	 * This is the client property of buttons created in static methods by this
	 * class.
	 */
	 ...
	 * Also this can be used as a dialog type, to indicate that "Cancel" should
	 * be the only option presented to the user.
	 * <P>
	 * Note the usage is similar to JOptionPane's, but the numerical value is
	 * different, so you cannot substitute JOptionPane.CANCEL_OPTION for
	 ...
	 * Also this can be used as a dialog type, to indicate that "OK" should be
	 * the only option presented to the user.
	 * <P>
	 * Note the usage is similar to JOptionPane's, but the numerical value is
	 * different, so you cannot substitute JOptionPane.OK_OPTION for
	 ...
	 * Also this can be used as a dialog type, to indicate that "No" should be
	 * the only option presented to the user.
	 * <P>
	 * Note the usage is similar to JOptionPane's, but the numerical value is
	 * different, so you cannot substitute JOptionPane.NO_OPTION for
	 ...
	 * Also this can be used as a dialog type, to indicate that "Yes" should be
	 * the only option presented to the user.
	 * <P>
	 * Note the usage is similar to JOptionPane's, but the numerical value is
	 * different, so you cannot substitute JOptionPane.YES_OPTION for
	 ...
	 * <P>
	 * Note the usage is similar to JOptionPane's, but the numerical value is
	 * different, so you cannot substitute JOptionPane.YES_NO_OPTION for
	 * DialogFooter.YES_NO_OPTION.
	 */
	public static final int YES_NO_OPTION = uniqueCtr++;

	/**
	 * Used to indicate a dialog should present a "Yes", "No", and "Cancel"
	 * option.
	 * <P>
	 * Note the usage is similar to JOptionPane's, but the numerical value is
	 * different, so you cannot substitute JOptionPane.YES_NO_CANCEL_OPTION for
	 * DialogFooter.YES_NO_CANCEL_OPTION.
	 */
	 ...
	 * <P>
	 * Note the usage is similar to JOptionPane's, but the numerical value is
	 * different, so you cannot substitute JOptionPane.OK_CANCEL_OPTION for
	 * DialogFooter.OK_CANCEL_OPTION.
	 */
	public static final int OK_CANCEL_OPTION = uniqueCtr++;

	/**
	 * Used to indicate a dialog should present a "Save", "Don't Save", and
	 * "Cancel" option.
	 */
	public static final int SAVE_DONT_SAVE_CANCEL_OPTION = uniqueCtr++;

	/**
	 * Used to indicate a dialog should present a "Don't Save" and "Save"
	 * option. This will be used for QOptionPaneCommon.FILE_EXTERNAL_CHANGES.
	 */
	public static final int DONT_SAVE_SAVE_OPTION = uniqueCtr++;

	/**
	 * Used to indicate the user selected "Save" in a dialog. <BR>
	 * Also this can be used as a dialog type, to indicate that "Save" should be
	 * the only option presented to the user.
	 */
	 ...
	 * Also this can be used as a dialog type, to indicate that "Don't Save"
	 * should be the only option presented to the user.
	 */
	public static final int DONT_SAVE_OPTION = uniqueCtr++;

	/**
	 * Used to indicate the user selected an option not otherwise specified in
	 * this set of constants. It may be possible that the user closed this
	 * dialog with the close decoration, or else another agent dismissed this
	 * dialog.
	 * <p>
		...
		private void setRootPaneContainer(JButton button, RootPaneContainer c) {
			...
			if (c != null) {
				c.getRootPane()
						.getInputMap(
								JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
						.put(escapeKey, escapeKey);
				c.getRootPane().getActionMap()
						.put(escapeKey, new ClickAction(button));
	...
	/**
	 * This guarantees that when the escape key is pressed (if its parent window
	 * has the keyboard focus) this button is clicked.
	 * <p>
	 * It is assumed that no two buttons will try to consume escape keys in the
	 * same window.
	 * 
	 * @param button
	...
	/**
	 * Creates a new "Don't Save" button that is not triggered by the escape
	 * key.
	 */
	...
	/**
	 * Creates a <code>DialogFooter</code> and assigns a default button. The
	 * default button is the first button listed in the button type. For
	 * example, a YES_NO_CANCEL_OPTION dialog will make the YES_OPTION the
	 * default button.
	 * 
	 * @param options
	 *            one of the OPTIONS fields in this class, such as YES_NO_OPTION
	 *            or CANCEL_OPTION.
	 * @param escapeKeyBehavior
	 *            one of the EscapeKeyBehavior options in this class.
	 * @return a <code>DialogFooter</code>
	 */
	public static DialogFooter createDialogFooter(int options,
			EscapeKeyBehavior escapeKeyBehavior) {
		return createDialogFooter(new JComponent[] {}, options,
				escapeKeyBehavior);
	}
	 ...
	 * default button.
	 * <P>
	 * To use a different default button, use the other
	 * <code>createDialogFooter()</code> method.
	 * 
	...
	public static DialogFooter createDialogFooter(JComponent[] leftComponents,
			int options, EscapeKeyBehavior escapeKeyBehavior) {
		if (options == CANCEL_OPTION) {
			return createDialogFooter(leftComponents, options, CANCEL_OPTION,
					escapeKeyBehavior);
		} else if (options == DONT_SAVE_OPTION) {
			return createDialogFooter(leftComponents, options,
					DONT_SAVE_OPTION, escapeKeyBehavior);
		} else if (options == NO_OPTION) {
			return createDialogFooter(leftComponents, options, NO_OPTION,
					escapeKeyBehavior);
			...
			return createDialogFooter(leftComponents, options, SAVE_OPTION,
					escapeKeyBehavior);
			...
			return createDialogFooter(leftComponents, options, YES_OPTION,
					escapeKeyBehavior);
	 ...
	 *            or CANCEL_OPTION.
	 * @param defaultButton
	 *            the OPTION field corresponding to the button that should be
	 *            the default button, or -1 if there should be no default
	 *            button.
	 * @param escapeKeyBehavior
	 *            one of the EscapeKeyBehavior options in this class.
	 * @return a <code>DialogFooter</code>
	 */
	public static DialogFooter createDialogFooter(JComponent[] leftComponents,
			int options, int defaultButton, EscapeKeyBehavior escapeKeyBehavior) {
		...
		if (escapeKeyBehavior == EscapeKeyBehavior.TRIGGERS_NONDEFAULT) {
			int buttonCount = 1;
			if (options == OK_CANCEL_OPTION || options == YES_NO_OPTION
					|| options == DONT_SAVE_SAVE_OPTION) {
				buttonCount = 2;
			} else if (options == SAVE_DONT_SAVE_CANCEL_OPTION
			...
			if (buttonCount > 1) {
				throw new IllegalArgumentException(
						"request for escape key to map to " + buttonCount
								+ " buttons.");
			}
		}

		if (options == CANCEL_OPTION || options == OK_CANCEL_OPTION
			...
			cancelButton = createCancelButton(escapeKeyBehavior == EscapeKeyBehavior.TRIGGERS_CANCEL
					...
					|| (defaultButton == CANCEL_OPTION && escapeKeyBehavior == EscapeKeyBehavior.TRIGGERS_DEFAULT));
		}
		if (options == DONT_SAVE_OPTION
				|| options == SAVE_DONT_SAVE_CANCEL_OPTION
				|| options == DONT_SAVE_SAVE_OPTION) {
			dontSaveButton = createDontSaveButton((escapeKeyBehavior == EscapeKeyBehavior.TRIGGERS_NONDEFAULT && defaultButton != DONT_SAVE_OPTION)
					|| (escapeKeyBehavior == EscapeKeyBehavior.TRIGGERS_DEFAULT && defaultButton == DONT_SAVE_OPTION));
		}
		if (options == NO_OPTION || options == YES_NO_OPTION
				|| options == YES_NO_CANCEL_OPTION) {
			noButton = createNoButton((escapeKeyBehavior == EscapeKeyBehavior.TRIGGERS_NONDEFAULT && defaultButton != NO_OPTION)
		...
		if (options == OK_OPTION || options == OK_CANCEL_OPTION) {
			okButton = createOKButton((escapeKeyBehavior == EscapeKeyBehavior.TRIGGERS_NONDEFAULT && defaultButton != OK_OPTION)
					|| (escapeKeyBehavior == EscapeKeyBehavior.TRIGGERS_DEFAULT && defaultButton == OK_OPTION));
		}
		if (options == SAVE_OPTION || options == SAVE_DONT_SAVE_CANCEL_OPTION
				|| options == DONT_SAVE_SAVE_OPTION) {
			saveButton = createSaveButton((escapeKeyBehavior == EscapeKeyBehavior.TRIGGERS_NONDEFAULT && defaultButton != SAVE_OPTION)
					|| (escapeKeyBehavior == EscapeKeyBehavior.TRIGGERS_DEFAULT && defaultButton == SAVE_OPTION));
		}
		if (options == YES_OPTION || options == YES_NO_OPTION
				|| options == YES_NO_CANCEL_OPTION) {
			yesButton = createYesButton((escapeKeyBehavior == EscapeKeyBehavior.TRIGGERS_NONDEFAULT && defaultButton != YES_OPTION)
		...
		if (options == CANCEL_OPTION) {
			...
			dismissControls = new JButton[] { saveButton, dontSaveButton,
					cancelButton };
		} else if (options == SAVE_OPTION) {
		...
		for (int a = 0; a < dismissControls.length; a++) {
			int i = ((Integer) dismissControls[a]
					.getClientProperty(PROPERTY_OPTION)).intValue();
			if (i == defaultButton)
				theDefaultButton = dismissControls[a];
		}

		DialogFooter footer = new DialogFooter(leftComponents, dismissControls,
				true, theDefaultButton);
		return footer;
	}
	...
	/**
	 * This indicates whether the dismiss controls should be displayed in
	 * reverse order. When you construct a DialogFooter, the dismiss controls
	 * should be listed in order of priority (with the most preferred listed
	 * first, the least preferred last). If this boolean is false, then those
	 * components will be listed in that order. If this is true, then those
	 * components will be listed in the reverse order.
	 * <P>
	 * By default on Mac this is true, because Macs put the default button on
	 * the right side of dialogs. On all other platforms this is false by
	 * default.
	 * <P>
	 * Window's <A
	 * HREF="http://msdn.microsoft.com/en-us/library/ms997497.aspx">guidelines
	 * </A> advise to, "Position the most important button -- typically the
	 * default command -- as the first button in the set."
	 */
	...
	private final HierarchyListener hierarchyListener = new HierarchyListener() {
		...
		private void processRootPane() {
			...
			for (int a = 0; a < dismissControls.length; a++) {
				if (dismissControls[a] instanceof JButton) {
					Character ch = (Character) dismissControls[a]
							.getClientProperty(PROPERTY_META_SHORTCUT);
					if (ch != null) {
						KeyStroke keyStroke = KeyStroke.getKeyStroke(ch
								.charValue(), Toolkit.getDefaultToolkit()
								.getMenuShortcutKeyMask());
						root.getInputMap(
								JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
								.put(keyStroke, keyStroke);
						root.getActionMap().put(keyStroke,
								new ClickAction((JButton) dismissControls[a]));
					}
				}
			}
		}
		...
		private void processWindow() {
			...
			window.setFocusTraversalPolicy(new DelegateFocusTraversalPolicy(
					window.getFocusTraversalPolicy()) {
				private static final long serialVersionUID = 1L;

				@Override
				public Component getDefaultComponent(Container focusCycleRoot) {
					/**
					 * If the default component would naturally be in the footer
					 * *anyway*: Make sure the default component is the default
					 * button.
					 * 
					 * However if the default component lies elsewhere (a text
					 * field or check box in the dialog): that should retain the
					 * default focus.
					 * 
					 */
					...
					if (DialogFooter.this.isAncestorOf(defaultComponent)) {
						...
						if (button != null && button.isShowing()
								&& button.isEnabled() && button.isFocusable())
							return button;
					}
	 ...
	 * 
	 * @param leftControls
	 *            the controls on the left side of this dialog, such as a help
	 *            component, or a "Reset" button.
	 * @param dismissControls
	 *            the controls on the right side of this dialog that should
	 *            dismiss this dialog. Also called "action" buttons.
	 * @param autoClose
	 *            whether the dismiss buttons should automatically close the
	 *            containing window. If this is <code>false</code>, then it is
	 *            assumed someone else is taking care of closing/disposing the
	 *            containing dialog
	 * @param defaultButton
	 *            the optional button in <code>dismissControls</code> to make
	 *            the default button in this dialog. (May be null.)
	 */
	public DialogFooter(JComponent[] leftControls,
			JComponent[] dismissControls, boolean autoClose,
		...
		for (int a = 0; a < dismissControls.length; a++) {
			dismissControls[a].putClientProperty("dialog.footer.index",
					new Integer(a));
			if (dismissControls[a] instanceof JButton) {
				((JButton) dismissControls[a])
						.addActionListener(innerActionListener);
			} else {
				// think of things like the JLink: it a label, but it has an
				// ActionListener model
				try {
					Class<?> cl = dismissControls[a].getClass();
					Method m = cl.getMethod("addActionListener",
							new Class[] { ActionListener.class });
					m.invoke(dismissControls[a],
							new Object[] { innerActionListener });
				} catch (Throwable t) {
					// do nothing
				}
			}
	...
	public void setInternalButtonPadding(int widthPadding, int heightPadding) {
		if (buttonWidthPadding == widthPadding
	...
	private void installGUI() {
		...
		for (int a = 0; a < unsafeButtons.length; a++) {
			JComponent comp = reverseButtonOrder ? unsafeButtons[unsafeButtons.length
					- 1 - a]
					: unsafeButtons[a];
			add(comp, c);
			c.gridx++;
		...
		for (int a = 0; a < safeButtons.length; a++) {
			JComponent comp = reverseButtonOrder ? safeButtons[safeButtons.length
					- 1 - a]
					: safeButtons[a];
	...
	/**
	 * This takes a set of buttons and gives them all the width/height of the
	 * largest button among them.
	 * <P>
	 * (More specifically, this sets the <code>preferredSize</code> of each
	 * button to the largest preferred size in the list of buttons.
	 * 
	 * @param buttons
	 *            an array of buttons.
	 */
	 ...
	 * 
	 * @param l
	 *            this listener will be notified when a
	 *            <code>dismissControl</code> is activated.
	 */
	 ...
	 * <P>
	 * Note the components on the left side of this footer (such as a "Help"
	 * button or a "Reset Preferences" button) do NOT dismiss the dialog, and so
	 * this method has nothing to do with those components. This only relates to
	 * the components on the right side of dialog.
	 * 
	 ...
	 * <P>
	 * If this footer is recycled in different dialogs, then you may need to
	 * nullify this value for <code>getLastSelectedComponent()</code> to remain
	 * relevant.
	 */
	public void reset() {
	...
	/**
	 * Returns a copy of the <code>dismissControls</code> array used to
	 * construct this footer.
	 */
	...
	/**
	 * Returns a copy of the <code>leftControls</code> array used to construct
	 * this footer.
	 */
