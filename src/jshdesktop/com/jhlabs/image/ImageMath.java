...
package jshdesktop.com.jhlabs.image;
...
public class ImageMath {
	 ...
	 *            the number to bias
	 * @param b
	 *            the bias parameter. 0.5 means no change, smaller values bias
	 ...
	 * @param b
	 *            the gain parameter. 0.5 means no change, smaller values reduce
	...
	public static float gain(float a, float b) {
		 ...
		 * 
		 * if (a < .001) return 0.0f; else if (a > .999) return 1.0f; if (a <
		 * 0.5) return (float)Math.pow(2 * a, p) / 2; else return 1.0f -
		 * (float)Math.pow(2 * (1. - a), p) / 2;
		 */
